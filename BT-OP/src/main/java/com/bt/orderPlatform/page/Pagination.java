...
public class Pagination {
	...
	*/
	public static final String getPageView(int pageSize,int totleSize,int totlePage,int pageIndex) {
		...
		sb.append("<li>");
		sb.append("<input id='jumIndex' type='hidden' style='height:32px;' value='1' onchange='KeyDown()'/>");
...
//		}
//		sb.append("</select>");
		sb.append("<select id='jumpPageSize' class='no-margin' style='width:70px;height:32px;' onchange='KeyDown()'>");
		if(pageSize==25){
			sb.append("<option selected=selected>25</option>");
		}else{
		...
		if(pageSize==150){
			sb.append("<option selected=selected>150</option>");
		}else{
			sb.append("<option>150</option>");
		}
		if(pageSize==200){
			sb.append("<option selected=selected>200</option>");
		}else{
		...
		result.append(sb);
		result.append("</div><div style='float:right;line-height:30px;'><span>共"+totleSize+"条记录"+getTotlePage(totleSize, totlePage, pageSize)+"页</span>&nbsp;&nbsp;</div>");
		return result.toString();

	}
	public static StringBuffer getScript(StringBuffer sb,int pageIndex,int pageSize,int totleSize){
		sb.append("<script type='text/javascript'>");
		sb.append("function setData(type){$(window).scrollTop(0);");
		...
		sb.append("function jumpToPage(index){$(window).scrollTop(0);");
		...
		sb.append("function KeyDown(){$(window).scrollTop(0);");
