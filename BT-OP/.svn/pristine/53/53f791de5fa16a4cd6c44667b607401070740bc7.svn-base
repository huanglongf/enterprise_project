...
<script type="text/javascript">
	$(function(){
		//
		if($(".table-content-free").height() < (totalHeight - $(".search-toolbar").height() - 40)) {
			$(".table-content-free").height(totalHeight - $(".search-toolbar").height() - 40);
		}
		//
		hideYScroll("#templet_test_content", mainTableWidth);
		//
			  		...
			  		<tr>
			  			<c:if test="${not empty tableColumnConfig }">
			  				<th class="text-center table-text col-sm">
								<input type="checkbox" id="checkAll_templetTest" onclick="inverseCkb('ckb_templetTest')" />
							</th>
			  			</c:if>
							...
							<tr class="pointer" ondblclick='tableAction($(this), ${tableFunctionConfig });'>
								<c:if test="${not empty tableColumnConfig }">
					  				<td class="text-center table-text col-sm">
										<input type="checkbox" id="ckb_templetTest" name="ckb_templetTest" value="${res.id }" data-version="${res.version }" />
									</td>
					  			</c:if>
					  			<c:forEach items="${tableColumnConfig }" var="tableColumnConfig">
...
<div class="dataTables_wrapper form-inline">
	<div class="row">
		<div class="col-md-12">
			<a class="pointer" title="列表配置" onclick='tableconfig(${tableFunctionConfig });'><i class="fa fa-cog fa-2x fa-spin"></i></a>
			${pageView.pageView }
		</div>
	</div>
</div>