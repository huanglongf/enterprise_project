...
<html>
	...
	<body>
			<div class="page-header"><h1 style='margin-left:20px'>运单</h1>
			</div>
									...
									<div class="widget-main no-padding-bottom">
										<form id="query_logistics" name="query_logistics" class="container search_form">
											<div class="row form-group">
												...
												<div class="col-md-3 no-padding">
													<div class="col-md-11 no-padding">
														<select id="expressCode" name="expressCode" data-edit-select="1" style="width: 160px;height: 34px;">
											  				<option value= "" select>---请选择---</option>
											  				  <c:forEach items= "${venders}" var= "street" >
											  					    <%--  <option value="${street.express_code}">${street.express_name}</option> --%>
												  					 <c:if test='${queryParam.expressCode eq street.express_code }'> 
																		<option value="${street.express_code}" ${street.express_code ==  queryParam.expressCode ? "selected = 'selected'" : "" }>${street.express_name}</option>
																	 </c:if>
																	 <c:if test='${queryParam.expressCode ne street.express_code }'> 
																		<option  value="${street.express_code}">${street.express_name}</option>
																	 </c:if>
															 </c:forEach> 
														</select>
											...
											<div class="row form-group">
												...
												<div class="col-md-3 no-padding">
													<div class="col-md-11 no-padding">
														<select id="producttype_name" name="producttype_code" data-edit-select="1" style="width: 160px;height: 34px;display: none;">
											  					<option value= "" select>---请选择---</option>
												  				  <c:forEach items= "${type}" var= "street" >
												  					    <%-- <option value="${street.product_type_code}">${street.product_type_name}</option> --%>
													  					 <c:if test='${queryParam.producttype_code eq street.product_type_code }'> 
																			<option value="${street.product_type_code}" ${street.product_type_code ==  queryParam.producttype_code ? "selected = 'selected'" : "" }>${street.product_type_name}</option>
																		 </c:if>
																		 <c:if test='${queryParam.producttype_code ne street.product_type_code }'> 
																			<option  value="${street.product_type_code}">${street.product_type_name}</option>
																		 </c:if>
																 </c:forEach> 
														</select>
											...
											<div class="row form-group">
												...
												<div class="col-md-3 no-padding">
													<div class="col-md-11 no-padding">
														<input type='text' id='memo' name='memo' value="${queryParam.memo}" style="width: 160px;height: 34px;">
													</div>
												</div>
											</div>
											<div class="row form-group">
												...
												<div class="col-md-3 no-padding">
													<div class="col-md-11 no-padding">
														<input type='text'id='ljs_time' name='ljs_time' value="${queryParam.ljs_time}" style="width: 160px;height: 34px;"onFocus="WdatePicker({startDate:'%y-%M-01 00:00:00',dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:true})">
													</div>
												</div>
												...
												<div class="col-md-3 no-padding">
													<div class="col-md-11 no-padding input-group">
														<input type='text'id='lje_time' name='lje_time' value="${queryParam.lje_time}" style="width: 160px;height: 34px;"onFocus="WdatePicker({startDate:'%y-%M-01 00:00:00',dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:true})">
													</div>
												</div>
												<div class="col-md-1 no-padding text-center search-title">
											...
											<div class="row form-group">
														...
														<select id="query_to_orgnization" name="to_orgnization" data-edit-select="1" style="width: 160px;height: 34px;">
											  				<option value= "" select>---请选择---</option>
											  				  <c:forEach items= "${orgs}" var= "street" >
											  					  ...
											  					  <c:if test='${queryParam.to_orgnization eq street.org_name }'> 
												...
												<div class="col-md-3 no-padding">
													<div class="col-md-11 no-padding input-group">
														<select id="query_org_provice_code" name="to_province_code" data-edit-select="1" style="width: 160px;height: 34px;" onchange='shiftArea(1,"query_org_provice_code","query_org_city_code","query_org_state_code","")'>
											  				<option value= "" select>---请选择---</option>
											  				<c:forEach items="${areas}" var = "area" >
															    <c:if test='${queryParam.to_province_code eq area.area_code }'> 
																	...
																	<option value="${area.area_code}" ${area.area_code ==  queryParam.to_province_code ? "selected = 'selected'" : "" }>${area.area_name}</option>
																 </c:if>
											...
											<div class="row form-group">
												...
												<div class="col-md-3 no-padding">
													<div class="col-md-11 no-padding">
														<input type='text' id='query_to_contacts' name='to_contacts' value="${queryParam.to_contacts}" style="width: 160px;height: 34px;">
													</div>
												</div>
												...
												<div class="col-md-3 no-padding">
													<div class="col-md-11 no-padding input-group">
														<input type='text' id='query_to_phone' name='to_phone' value="${queryParam.to_phone}" style="width: 160px;height: 34px;">
													</div>
												</div>
												<div class="col-md-1 no-padding text-center search-title">
												...
												<div class="col-md-3 no-padding">
													<div class="col-md-11 no-padding input-group">
														<input type='text' id='query_to_address' name='to_address'  value="${queryParam.to_address}" style="width: 160px;height: 34px;">
													</div>
												</div>
											</div>
											</div>
								...
								<div class="row">
									<div class="col-md-12">
										<div class="dataTables_length">
											<button class="btn btn-sm btn-white btn-default btn-bold btn-round btn-width" style= "height: 30px; width: 120px; " onclick="queryLogistics();">
												<i class="icon-search icon-on-right bigger-120"></i>查询
											</button> 
											<button class="btn btn-sm btn-white btn-default btn-bold btn-round btn-width" style= "height: 30px; width: 120px; " onclick="addlogistics();">
											...
											<button class="btn btn-sm btn-white btn-default btn-bold btn-round btn-width" style= "height: 30px; width: 125px; " onclick="csvexportwmd();">
				...
				<tbody  align="center">
			  		<c:forEach items= "${pageView.records }" var= "records" varStatus='status'>
				  		<tr ondblclick='toUpdate("${records.id}")'>
							...
							<td class="td_cs" style="width: 200px;height: 72px" title="${records.create_time }">
		...
		function addlogistics() {
			loadingCenterPanel('${root }/control/orderPlatform/waybillMasterController/addlogistics.do?time='+new Date().getTime());
		}
		function toUpdate(uuid) {
			loadingStyle();
			loadingCenterPanel('${root }/control/orderPlatform/waybillMasterController/updatelogistics.do?id='+uuid);
			cancelLoadingStyle();
		}
		        	            ...
		        	            if(data.data==1){
		        	            	if(data.code==4){
		        	            		 ...
		        	            		 if(result1){
		        	            			 $.ajax({
		        	            					type : "POST",
		        	            					url: root+"control/orderPlatform/SFPrintController/printBaozunWaybilMaster.do?ids="+idsStr, 
		        	            					data:"",
		        	            					dataType: "json",  
		        	            					success : function(jsonStr) {
		        	            			         if(jsonStr.out_result==1){
		        	            		...
		        	            		window.open("http://localhost/"+jsonStr.file_name);
		        	            	}
		        	            }else if(data.data==2){
		        	            	alert("操作失败！");
		        	            	alert(data.msg);
