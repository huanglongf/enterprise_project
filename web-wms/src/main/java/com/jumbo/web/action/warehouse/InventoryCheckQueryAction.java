...
public class InventoryCheckQueryAction extends BaseJQGridProfileAction implements ServletResponseAware {
    ...
    public String invCKQueryEntry() {
        invckStatuslist = chooseOptionManager.findOptionListByCategoryCode("inventoryCheckStatus");
        return SUCCESS;
    }

    public String findSnDiffLineByIc() {
        setTableConfig();
        Pagination<InventoryCheckDifferenceSnLineCommand> lines = warehouseManager.findSnDiffLineByIC(tableConfig.getStart(), tableConfig.getPageSize(), invcheckid, tableConfig.getSorts());
        request.put(JSON, toJson(lines));
        return JSON;
    }

    public String findSnDiffLineSkuChangeByIc() {
        setTableConfig();
        Pagination<InventoryCheckDifferenceSnLineCommand> lines = warehouseManager.findSnDiffLineSkuChangeByIc(tableConfig.getStart(), tableConfig.getPageSize(), invcheckid, tableConfig.getSorts());
        request.put(JSON, toJson(lines));
        return JSON;
    }

    public String findAllInventoryCheckList() {
        setTableConfig();
        if (iccommand != null) {
        ...
        Pagination<InventoryCheckCommand> inventoryCheckList = wareHouseManagerQuery.findAllInventoryCheckList(tableConfig.getStart(), tableConfig.getPageSize(), iccommand, this.userDetails.getCurrentOu().getId(), tableConfig.getSorts());
