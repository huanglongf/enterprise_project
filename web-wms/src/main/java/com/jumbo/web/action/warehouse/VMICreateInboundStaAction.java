...
public class VMICreateInboundStaAction extends BaseJQGridProfileAction {
    ...
    public String vmiCreateInboundSta() {
        return SUCCESS;
    }

    public String vmiInboundImport() {
        String msg = SUCCESS;
        request.put("msg", msg);
        ...
        if (file == null) {
            ...
            try {
                ReadStatus rs = coachParseDataManagerProxy.generateCoachInventoryImport(file, Long.parseLong(request.get("ownerid").toString()), request.get("slipCode").toString(), userDetails.getCurrentOu().getId(), userDetails.getUser().getId());
                if (rs != null) {
