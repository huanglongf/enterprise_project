...
function printByPrinterName(url1,url2){
	//获取可用打印机服务名称
	var printers = window.top.document.printApplet.getAllPrinterNames();
	alert(printers);
	var pickingListPrinterName=getCookieBZ("pickingListKey");
	var logisticsPrinterName=getCookieBZ("logisticsKey");
	if((pickingListKey==null)&&(url1!=null)){
		//需要判断值是否在可用范围内
		//弹框设置装箱单打印
		setCookieBZ("pickingListKey","aaaa"); 
	}
