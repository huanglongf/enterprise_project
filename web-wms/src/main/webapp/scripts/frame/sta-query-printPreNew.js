...
$j(document).ready(function (){
	...
	$j("#tbl-orderList").jqGrid({
		...
		datatype: "json",
		colNames: ["id","原物流商","原快递单号","新物流商","新快递单号","操作"],
		colModel: [         {name: "id", index: "id", hidden: true},
		                    {name:"oldLpcode", index:"oldLpcode", width:200, resizable:true},
							{name:"oldTrackingNo", index: "oldTrackingNo", width:200},
							{name:"lpCode", index:"lpCode", width:200, resizable:true},
							{name:"trackingNo", index: "trackingNo", width:200},
							{name:"idForBtn",index:"idForBtn", width: 250,resizable:true,sortable: false, formatter:"buttonFmatter", formatoptions:{"buttons":{label:"打印新的物流面单", onclick:"printDeliveryInfo(this)"}}}
					 	],
		caption: "数据列表",
	    multiselect: true,
	    sortname: 'i.ID',
	...
	$j("#search").click(function(){
		var url = baseUrl + "/findTrackingNoPre.json";
...
function printDeliveryInfo(obj){
	...
	if (null != ids && ids.length > 0) {
		    ...
		    jumbo.showMsg(i18("DELIVERYINFO"));
			var ids = ids+",";
			var url = window.$j("body").attr("contextpath") + "/printSingleOrderDetail1.json?id=" +ids;
