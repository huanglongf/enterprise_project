...
$j.extend(loxia.regional['zh-CN'],{
	...
	TABLE_CAPTION_STA				:	"采购待收货列表",
	TABLE_CAPTION_STALINE			:	"采购待收货明细表",
...
function showdetail(obj){
	...
	$j("#staId").val(staId);
	if(sta.intStaType == "55"){
		$j('#GI_div_import').addClass("hidden");
	} else {
		$j('#GI_div_import').removeClass("hidden");
	}
...
$j(document).ready(function (){
	...
	$j("#search").click(function(){
		...
		postData['sta.intType'] = 11;
		$j("#tbl-inbound-purchase").jqGrid('setGridParam',{
			url:$j("body").attr("contextpath")+"/stalist.json",postData:postData}).trigger("reloadGrid");
	});
	...
	$j("#snscode").keydown(function(evt){
		if(evt.keyCode === 13){
			...
			if(sns&&sns.length>0&&reg.test(sns)){
				...
				if(snmap[sns]) 
					jumbo.showMsg("输入的sn号： ' " + sns + " '" + "重复,请重新输入");
				else {
						snmap[sns] = sns;
						row["id"]=index;
						row["sns"]=sns;
	...
	if(!status.exception){
		...
		$j("#tbl-inbound-purchase").jqGrid({
			url:$j("body").attr("contextpath")+"/stalist.json?sta.intType=11",
			postData:loxia._ajaxFormToObj("form_query"),
			datatype: "json",
			colNames: ["ID","STVID","STVTOTAL","STVMODE",i18("ENTITY_STA_CREATE_TIME"),i18("ENTITY_STA_ARRIVE_TIME"),i18("ENTITY_STA_CODE"),
				i18("ENTITY_STA_PROCESS"),"STA_TYPE","作业类型",i18("ENTITY_STA_UPDATE_TIME"),i18("ENTITY_STA_PO"),
				i18("ENTITY_STA_OWNER"),//i18("ENTITY_STA_SUP"),i18("ENTITY_STA_SEND_MODE"),
				i18("ENTITY_STA_REMANENT"),i18("ENTITY_STA_COMMENT"),"IS_PDA"],
			colModel: [
			           {name: "id", index: "id", hidden: true},		         
			           {name: "stvId", index: "stvId", hidden: true},		         
			           {name: "stvTotal", index: "stv_total", hidden: true},		         
			           {name: "stvMode", index: "stv_mode", hidden: true},		         
			           {name: "createTime", index: "create_time", width: 130, resizable: true,sortable:false},
			           {name: "arriveTime", index: "arrive_time", width: 130, resizable: true,sortable:false},
			           {name: "code", index: "sta.code",formatter:"linkFmatter", formatoptions:{onclick:"showdetail"}, width: 100, resizable: true,sortable:false},
			           {name: "processStatus", index: "status", width: 100, resizable: true,
				        	   formatter:'select',editoptions:status,sortable:false},
				       {name: "intStaType", index: "intStaType", hidden: true},
				       {name: "staType", index: "intStaType", width: 100, resizable: true,sortable:false},
			           {name: "inboundTime", index: "inbound_time", width: 130, resizable: true,sortable:false},
		               ...
		               {name:"memo",index:"memo",width:100,resizable:true,sortable:false},
		               {name:"isPda",width:150,hidden: true}],
			caption: i18("TABLE_CAPTION_STA"),
			//postData:{"columns":"id,stvId,stvTotal,stvMode,createTime,arriveTime,code,processStatus,inboundTime,refSlipCode,owner,remnant,memo,isPda"},
			...
			loadComplete:function(){
				...
				for(var i=0;i < ids.length;i++){
					...
					if(datas["intStaType"] == "11"){
						$j("#tbl-inbound-purchase").jqGrid('setRowData',ids[i],{"staType":"采购入库"});
					}else if(datas["intStaType"] == "55"){
						$j("#tbl-inbound-purchase").jqGrid('setRowData',ids[i],{"staType":"暂存区商品上架"});
					}
				}
			}
		});
	...
	$j("#receive,#receiveAll,#pdaReceive,#pdaReceiveAll").click(function(){
			...
			var postData=getPostData(isAll,null,$j("#staId").val());
			//PDA收货标记
					...
					postData,{
						success:function (data) {
							...
							if(data&&data.SNmsg){
								...
								window.location=loxia.getTimeUrl($j("body").attr("contextpath")+"/warehouse/purchaseentry.do");
							}else{
	...
	$j("#cancelSta").click(function(){
		var staId = $j("#staId").val()
		var data=$j("#tbl-inbound-purchase").jqGrid("getRowData",staId);
		var status = data["processStatus"];
		if(status != 1){
			jumbo.showMsg(i18("CAN_NOT_CANCEL"));
			return;
		}
		if(confirm(i18("CONFIRM_CANCEL"))){
			loxia.lockPage();
			loxia.asyncXhrPost($j("body").attr("contextpath") + "/cancelsta.json",
				{'sta.id': staId},
				{
				success:function (data) {
					loxia.unlockPage();
					jumbo.showMsg(i18("RESULT_SUCCESS"));
					window.location=loxia.getTimeUrl($j("body").attr("contextpath")+"/warehouse/purchaseentry.do");
				   },
				error:function(data){
					loxia.unlockPage();
					jumbo.showMsg(i18("RESULT_ERROR"));
				}
				});
		}
	});
	...
	$j("#importGI").click(function(){
		var file=$j.trim($j("#staGIFile").val()),errors=[];
		if(file==""||file.indexOf("xls")==-1){
			errors.push(i18("INVALID_MAND",i18("ENTITY_EXCELFILE")));
		}
		if(errors.length>0){
			...
			$j("#importGIForm").attr("action",
				loxia.getTimeUrl($j("body").attr("contextpath") 
					+ "/warehouse/staImportForGI.do?sta.id="+$j("#staId").val()));
			loxia.submitForm("importGIForm");
		}
	});
	
	$j("#exportGI").click(function(){
		var iframe = document.createElement("iframe");
		iframe.src = loxia.getTimeUrl($j("body").attr("contextpath") + "/warehouse/staGIExport.do?sta.id="+$j("#staId").val());
		iframe.style.display = "none";
		$j("#download").append($j(iframe));
	});
...
function tostvInfo(stvId,sta,isPda){
	...
	/*alert("staid: " + $j("#staId").val());
	alert("stvid: " + $j("#stvId").val());*/
	...
	if(isPda){
		...
		loxia.asyncXhrPost(window.parent.$j("body").attr("contextpath")+"/stvinfo.json",{	"stv.id":stvId},
				{
					success:function (data) {
					    if(data&&data.msg){
					    	loxia.unlockPage();
...
function getLocationsTable(line){
	...
	$j.each(line.stvLines,function(i, item) {
		//var url=$j("body").attr("contextpath") + "/findavaillocationforpurchase.json?stv.intMode="+intMode+"&sku.id="+line.skuId+"&stv.id="+$j("#stvId").val();
		tb.push("<tr id=\""+item.id+"\" skuId=\""+line.skuId+"\" staLineId=\""+line.stalineId+"\"  owner=\""+line.owner+"\" >");
		tb.push("<td><input type=\"checkbox\" /></td>");
		var locCode = item.locationCode == null ? "" : item.locationCode;
		tb.push("<td width=\"150\"><input loxiaType=\"input\"  value=\""+ locCode +"\" checkmaster=\"checklocation\" aclist=\""+url+"\" /></td>");
		...
		loxia.asyncXhrPost(window.parent.$j("body").attr("contextpath")+"/json/executeinventory.json",postData,
		{
			success:function (data) {
				if(data && data.result != 'success'){
					...
					window.location=$j("body").attr("contextPath")+"/warehouse/purchaseentry.do";
				}	
			}, 
	       error:function(){
				 jumbo.showMsg(i18("MSG_FAILURE"));	
