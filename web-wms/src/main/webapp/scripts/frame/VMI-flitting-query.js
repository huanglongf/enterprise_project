$j.extend(loxia.regional['zh-CN'],{
	...
	OPERATE_SUCCEE : "操作成功",
	OPERATE_FAIL : "操作失败",
	BETWEENLIBRARY_MOVE_LIST_CAPTION : "VMI调拨单列表",
	CONFIRM_EXEC : "您确定要执行？",
...
function showDetail(obj){
	...
	$j("#owner_OuName").html(vals["ownerOuName"]);
	$j("#addiOwner_OuName").html(vals["addiOwnerOuName"]);
...
function doCancel(id){
	...
	if(id != ''){
		...
		var postData = {}; 
		postData['sta.id'] = id;
		var rs = loxia.syncXhrPost(baseUrl + "/cancelVMIFlittingSta.json",postData);
		if(rs && rs.rs=='success'){
			jumbo.showMsg(i18("OPERATE_SUCCEE"));
			$j("#tbl-query-info").jqGrid('setGridParam',{url: baseUrl + "/queryVMIFlittingSta.json"}).trigger("reloadGrid",[{page:1}]);
...
$j(document).ready(function (){
	...
	$j("#tbl-query-info").jqGrid({
		url: $j("body").attr("contextpath")+"/json/queryVMIFlittingSta.json",
		datatype: "json",
		colNames: ["ID","STATUS","intStatus",i18("ENTITY_STA_CODE"),i18("STATUS"),"源店铺","目标店铺",i18("MAIN_WAREEHOUSE"),i18("ADDI_WAREEHOUSE"),i18("ENTITY_STA_CREATERUSER"),i18("OPERATE_TIME"),i18("OPERATE")],
		colModel: [
				...
				{name: "ownerOuName", index: "ownerOuName",width: "120"},
				{name: "addiOwnerOuName", index: "addiOwnerOuName",width: "120"},
				{name: "mainName", index: "mainName",width: "120"},
				{name: "addiName", index: "addiName",width: "120"},
				{name: "creater", index: "user.user_name",width: "100", resizable: true},
    	...
    	multiselect: false,
		sortorder: "desc",
		postData:{"columns":"id,status,code,intStatus,ownerOuName,addiOwnerOuName,mainName,addiName,creater,finishTime"},
...
//				var id =$j(this).parents("tr").attr("id");
	...
	$j("#search").click(function(){
		$j("#tbl-query-info").jqGrid('setGridParam',
			{url: $j("body").attr("contextpath")+"/json/queryVMIFlittingSta.json",postData:loxia._ajaxFormToObj("staForm")}).trigger("reloadGrid",[{page:1}]);
		jumbo.bindTableExportBtn("tbl-query-info",{"intStatus":"whSTAStatus"}, $j("body").attr("contextpath")+"/json/queryVMIFlittingSta.json",loxia._ajaxFormToObj("staForm"));
	});
...
//		url:"warehouse-other-operate-query-details.json",
		...
		colModel: [{name: "id", index: "id", hidden: true},
			...
			{name:"jmcode", index:"jmskucode" ,width:100,resizable:true},
			{name:"keyProperties", index:"jmskucodef" ,width:100,resizable:true},
			{name:"owner", index:"owner" ,width:180,resizable:true},
			{name:"quantity",index:"cost",width:"60",resizable:true},
	...
	$j("#execute,#cancel").click(function(){
		if(windows.confirm(i18("CONFIRM_EXEC_OPERATE"))){
			$j("#searchCondition").removeClass("hidden");
			$j("#details").addClass("hidden");
		}
	});
