...
//页面加载完绑定方法
var barCodeList;
var rswh;//仓库基本信息 是否允许手动称重getByOuId
$j(document).ready(function(){
	...
	$j("#pickinglist-table").jqGrid({
		datatype: "json",
		colNames: ["ID",i18("BATCHCODE"),i18("LPCODE"),i18("STATUS"),i18("PLANSTA"),i18("PLANSKU"),i18("CHECKEDSTA"),i18("CHECKEDSKU"),i18("CREATETIME"),"handId","仓库id"],
		colModel: [{name: "id", index: "id", hidden: true},		         
		           ...
		           {name: "whid",index:"whid",hidden:true}],
	...
	$j("#search").click(function(){
		...
		var cenid=$j("#selTrans").val();
			...
			$j("#pickinglist-table").jqGrid('setGridParam',{
				url:window.$j("body").attr("contextpath")+"/getallseckillpickinglistbystatusopc.json",
				postData:postData,
				page:1
			}).trigger("reloadGrid");
	});
...
//			   			barcodeList = loxia.syncXhr($j("body").attr("contextpath") + "/findskuBarcodeList.json",postData);
				...
				if(bc1==bc||bc1=="00"+bc){//主条码相等
					...
					for(var v in barcodeList[bc1]){
						if(barcodeList[bc1][v] == bc || barcodeList[bc1][v] == ('00'+bc) ){//子条码相等
							...
							if(pardeInt(num)+inQty>d.quantity){
								$j("#barCode").val("");
								$j("#barCode0").val("");
								$j("#flag").val("barCode");
								var msg = item.skuName+" 您要核对量大于计划执行量，请重新核对！";
	...
	$j("#pickId").keydown(function(event){
		if(event.keyCode == 13){
			event.preventDefault();
			var pickid=$j("#pickId").val();
			var postData={};
			postData["pickcode"]=pickid;
			rswh = loxia.syncXhrPost(loxia.getTimeUrl($j("body").attr("contextpath") + "/findwarehousebypickid.json"),postData);
			...
			if(code==""){
				...
				if(rs&&rs.result&&rs.result=="success"){
					if(rs&&rs["pickingList"]){
						$j("#whid").val(rs["pickingList"["whid"]]);
...
function showStaLine(tag){
	var cenid=$j(tag).parents("tr").children("td:eq(11)").attr("title");
	var postData={};
	postData["ou.id"]=cenid;
	rswh = loxia.syncXhrPost(loxia.getTimeUrl($j("body").attr("contextpath") + "/findwarehousebaseinfoopc.json"),postData);
	if (rswh.warehouse.isNeedWrapStuff){
		$j("#p1").removeClass("hidden");
		$j("#p2").removeClass("hidden");
		$j("#p_tr").removeClass("hidden");
	...
	$j("#whid").val($j(tag).parents("tr").children("td:eq(11)").attr("title"));
...
function printHoInfo(id){
   var cenid=$j("#whid").val();
	...
	if(rs1&&rs1.rs=="success"){
		...
		var rs = loxia.syncXhrPost($j("body").attr("contextpath") + "/generhandoverlist.json",{"pickingList.id":id,"ou.id":cenid});
		if(rs&&rs.rs=="success"){
