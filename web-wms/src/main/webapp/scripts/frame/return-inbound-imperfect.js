...
function imperfectStv(staId){
	var result = loxia.syncXhrPost(window.$j("body").attr("contextpath")+"/json/imperfectStvLine.json?staId="+staId);
	for(var idx in result){
		var count = parseInt($j("#barCodeCount").val()||"1",10);
		var intStatusId=result[idx].intInvstatusId;
		var intInvstatusName=result[idx].intInvstatusName;
		var sn =result[idx].sn;
		var sku=getSkuRow(result[idx].barCode);
		if(!sku){
		...
		if(sku){
			...
			if(rowId == ""){
				...
				row['intInvstatusName'] = intInvstatusName;
...
function getPostData(staId){
	...
	var sta=$j("#tbl-inbound-purchase").jqGrid("getRowData",staId);
	postData['stv.sta.owner']=sta.owner;
	var validateSkuQty = {};
	$j.each($j("#tbl-orderNumber").jqGrid("getRowData"),function(i,row){
		var skuQtyKey = row["barCode"];
		var skuCompleteQty = (validateSkuQty[skuQtyKey] == undefined ? 0 : validateSkuQty[skuQtyKey]);
...
$j(document).ready(function (){
	...
	$j("#search").click(function(){
		...
		var url = $j("body").attr("contextpath")+"/json/newStalist.json";
		$j("#tbl-inbound-purchase").jqGrid('setGridParam',{
			url:url,postData:postData}).trigger("reloadGrid");
	});
	$j("#add_status").change(function(){
		addStatus();
		 var display =$j('#imperfectScan').css('display');
		 if(display=="inline"){
			 $j("#imperfectScan").focus();
		 }else{
			 $j("#addSkuScan").focus();
		 }
	});
	...
	$j("#imperfectLine").change(function(){
		 var display =$j('#factoryCode').css('display');
		    if(display=="inline"){
		    	$j("#factoryCode").focus();
		    }else{
		    	$j("#addSkuScan").focus();
	...
	if(!status.exception){
		...
		$j("#tbl-inbound-purchase").jqGrid({
			url:$j("body").attr("contextpath")+"/newStalist.json?sta.intType=41",
			async : false,
			datatype: "json",
	...
	$j("#barCode,#barCodeCount").keydown(function(evt){
		if(evt.keyCode === 13){
			...
			$j.each($j("#tbl-orderNumber").jqGrid("getRowData"),function(i,row){
				...
				if(skuQtyKey==barCode){
				if(skuCompleteQty <= 0){
					jumbo.showMsg("商品["+row["barCode"]+"]数量大于计划量");
					status=false;
					return ;
				}
				}
			...
			if(barcode!=""){
				...
				if(sku){
					...
					if(isNaN(addQuantity)||addQuantity<0){
						...
						if($j("#add_status option:selected").text()=="残次品"){
							if(channel.isImperfect==1){
								...
								$j("#imperfectLineScan").removeClass("hidden");
								$j("#imperfectScan").removeClass("hidden");
								$j("#add_statusScan").removeClass("hidden");
								$j("#add_imperfect").removeClass("hidden");
								$j("#add_imperfectLine").removeClass("hidden");
								$j("#imperfect").empty();
								...
								if(result==null||result.length==0){
									result = loxia.syncXhrPost(window.$j("body").attr("contextpath")+"/json/findWarehouseImperfectl.json");
								}
								
								for(var idx in result){
									$j("<option value='" + result[idx].id + "'>"+ result[idx].name +"</option>").appendTo($j("#imperfect"));
								}
								$j("#add_statusScan").focus();
							}
							/*else{
							...
							$j("#add_imperfect").addClass("hidden");
							$j("#add_imperfectLine").addClass("hidden");
							$j("#add_statusScan").removeClass("hidden");
							$j("#imperfectScan").addClass("hidden");
							$j("#imperfectLineScan").addClass("hidden");
							$j("#add_statusScan").focus();
						}
						
						$j("#addBarCode").removeClass("hidden");
						$j("#addSkuScan").removeClass("hidden");
						...
						$j("#supplierCode").val(sku["jmskuCode"]);
	...
	$j("#add_statusScan").keydown(function(evt){
		if(evt.keyCode === 13){
			var statusScan=$j("#add_statusScan").val();
			$j("#add_status option").each(function(){
				if($j(this).text()==statusScan){
	...
	$j("#imperfectScan").keydown(function(evt){
		if(evt.keyCode === 13){
			var statusScan=$j("#imperfectScan").val();
			$j("#imperfect option").each(function(){
				if($j(this).text()==statusScan){
					 $j(this).attr("selected", "selected");  
					 imperfectScan();
					 return;
	...
	$j("#imperfectLineScan").keydown(function(evt){
		if(evt.keyCode === 13){
			var statusScan=$j("#imperfectLineScan").val();
			$j("#imperfectLine option").each(function(){
				if($j(this).text()==statusScan){
					 $j(this).attr("selected", "selected");  
					    var display =$j('#factoryCode').css('display');
					    if(display=="inline"){
	...
	$j("#factoryCode").keydown(function(evt){
		if(evt.keyCode === 13){
			  var display =$j('#poId').css('display');
			    if(display=="inline"){
			    	$j("#poId").focus();
			    }else{
			    	$j("#addSkuScan").focus();
	...
	$j("#poId").keydown(function(evt){
		if(evt.keyCode === 13){
			   $j("#addSkuScan").focus();
		}
	});
	$j("#receiveAllScan").keydown(function(evt){
		if(evt.keyCode === 13){
			var statusScan=$j("#receiveAllScan").val();
			if(statusScan=="ok"||statusScan=="OK"){
				receiveAll();
			}
					 
				}
	});
	$j("#addSkuScan").keydown(function(evt){
		if(evt.keyCode === 13){
			var statusScan=$j("#addSkuScan").val();
			if(statusScan=="ok"||statusScan=="OK"){
				addsku();
			}
	...
	$j("#backto").click(function(){
		 window.location=loxia.getTimeUrl($j("body").attr("contextpath")+"/warehouse/returninboundentryimperfect.do");
	});
	...
	loxia.asyncXhrPost(window.parent.$j("body").attr("contextpath")+"/json/executeinventory.json",postData,
			{
				success:function (data) {
					if(data && data.result != 'success'){
						...
						jumbo.showMsg("入库成功");
						window.location=$j("body").attr("contextPath")+"/warehouse/returninboundentryimperfect.do";
					}	
				}, 
		       error:function(){
					 jumbo.showMsg(i18("MSG_FAILURE"));	
...
function resetSkuInfo(){
	...
	$j("#add_skuName").text("");
	$j("#add_status").val("").addClass("hidden");
	$j("#addSkuScan").val("").addClass("hidden");
	$j("#add_statusScan").val("").addClass("hidden");
	$j("#imperfectScan").val("").addClass("hidden");
	$j("#imperfectLineScan").val("").addClass("hidden");
	$j("#add_imperfect").addClass("hidden");
	$j("#add_imperfectLine").addClass("hidden");
	$j("#tip").addClass("hidden");
	$j("#addSku").addClass("hidden");
...
function addsku(){
	...
	if(barcode!=""){
		...
		if(sku){
			...
			var postData = {};
			var owner=$j("#owner3").val();
			var status=$j("#add_status").find("option:selected").text();
			if(status=="残次品"){
				
				var channel = loxia.syncXhrPost(window.$j("body").attr("contextpath")+"/json/getByName.do",{"owner":owner});//渠道信息
				if(channel.isImperfect==1){
					var imperfectName=$j("#imperfect").find("option:selected").text();
					if(imperfectName=="其他"){
						var memo=$j("#sta_memo").val();
						if(memo==""){
							jumbo.showMsg("残次类型为其他，备注不允许为空");
							return;
						}
					}
				}else{
			...
			if(channel.isImperfectPoId==1){
				...
				if(factoryCode==""){
					jumbo.showMsg("工厂编码不能为空");
					return;
				}else{
					postData['imperfectStv.factoryCode']=factoryCode;
				}
			}
			if(channel.isImperfectTime==1){
				 var poId=$j("#poId").val();
				 if(poId==""){
						jumbo.showMsg("下单日期不能为空");
						return;
					}else{
						postData['imperfectStv.poId']=poId;
					}
			}
			}
			postData['imperfectStv.barCode']=$j("#add_barCode").text();
			postData['imperfectStv.jmCode']=$j("#add_jmCode").text();
			postData['imperfectStv.name']=$j("#add_skuName").text();
			postData['imperfectStv.supplierCode']=$j("#supplierCode").val();
			postData['imperfectStv.statusName']=status;
			postData['imperfectStv.statusId.id']=$j("#add_status").find("option:selected").val();
			postData['imperfectStv.quantity	']=0;
			postData['imperfectStv.completeQuantity']=0;
			postData['imperfectStv.addQuantity']=(isNaN(count)?1:count);
			postData['imperfectStv.imperfectName']=$j("#imperfect").find("option:selected").text();
			postData['imperfectStv.imperfectLineName']=$j("#imperfectLine").find("option:selected").text();
			postData['imperfectStv.memo']=$j("#sta_memo").val();
			postData['imperfectStv.sta.id']=$j("#staId").val();
			if(isSnSku){
				postData['imperfectStv.sn']=barcode;
			}
			var rs = loxia.syncXhrPost(window.$j("body").attr("contextpath")+"/json/addImperfectStv.do",postData);
			if(rs && rs["msg"] == 'success'){
				var statusName=$j("#add_status").find("option:selected").text();
				if(statusName=="残次品"){
					var owner=$j("#owner3").val();
					var channel = loxia.syncXhrPost(window.$j("body").attr("contextpath")+"/json/getByName.do",{"owner":owner});//渠道信息
					if(channel.isImperfect==1){
						var id=rs["id"];
						var url=window.$j("body").attr("contextpath")+"/json/printSkuImperfectStv.do?imperfectId="+id;
						printBZ(loxia.encodeUrl(url),true);
					}
				}
				jumbo.showMsg("收货确认或打印");
			if(rowId == ""){
				rowId = sku['id'] + "_" + intStatusId;
				var row = sku;
				row['id']=rowId;
			...
			if(isSnSku){
				...
				postData["imperfectStv.skuSn"]=barcode;
				$j("#tbl-orderNumber tr[id="+rowId+"]").data("sns",sns);//增加sn
			}
			resetBarCodeForm(rowId);
			loxia.initContext($j("#tbl-orderNumber"));
...
function addStatus(){
	if($j("#add_status option:selected").text()=="残次品"){
		...
		if(channel.isImperfect==1){
			if(channel.isImperfectPoId==1){
				$j("#factory").removeClass("hidden");
				$j("#factoryCode").removeClass("hidden");
			}
			if(channel.isImperfectTime==1){
				$j("#poTime").removeClass("hidden");
				$j("#poId").removeClass("hidden");
			}
			$j("#add_imperfect").removeClass("hidden");
			$j("#add_imperfectLine").removeClass("hidden");
			$j("#imperfectScan").removeClass("hidden");
...
function receiveAll(){
	...
	if(postData['error']){
		...
		loxia.lockPage();
		loxia.asyncXhrPost($j("body").attr("contextpath") + "/purchaseReceiveStepImperfect.json",
			postData,
			{
			success:function (data) {
				loxia.unlockPage();
