...
$j(document).ready(function (){
	...
	//获取仓库
	var rsBrand = loxia.syncXhr($j("body").attr("contextpath") + "/findAllWhByType.json",{});
	$j("#brandSelect").append("<option></option>");
	$j("#brandSelect1").append("<option></option>");
	for(var i = 0 ; i < rsBrand.warelist.length ; i++){
		$j("#brandSelect").append("<option>"+rsBrand.warelist[i].name+"</option>");
		$j("#brandSelect1").append("<option>"+rsBrand.warelist[i].name+"</option>");
	}
	$j("#brandSelect1").flexselect();
	$j("#brandSelect1").val("");
	...
	// 打印收货单明细：
	$j("#printInboundDetail").click(function(){
		loxia.lockPage();
		var url = $j("body").attr("contextpath") + "/printInboundDetail.json?sta.id="+$j("#sta_id").val();
	...
	$j("#import").click(function(){
		...
		if(errors.length>0){
			...
			$j("#importForm").attr("action",
				loxia.getTimeUrl($j("body").attr("contextpath") + "/warehouse/importStaInboundTurn.do?mainWarehouseName="+wid));
	...
	$j("#confirm").click(function(){
		var id=$j("#sta_id").val();
		var pl=$j("#tbl_sta_list").jqGrid("getRowData",id);
		var wid=$j("#brandSelect").val();
		loxia.asyncXhrPost($j("body").attr("contextpath") + "/updateStaOuId.json",
				{"staCode":pl["code"],"mainWarehouseName":wid},{
			success:function (data) {
					if(data.result=="error"){
						jumbo.showMsg("作业单："+pl["code"]+"仓库名称"+wid+"更新失败!<br/>");
					}else{
						jumbo.showMsg("转仓成功！");
