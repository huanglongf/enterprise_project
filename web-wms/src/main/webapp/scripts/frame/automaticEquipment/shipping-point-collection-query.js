...
$j(document).ready(function (){
	...
	$j("#tbl-shipping-point").jqGrid({
		...
		datatype: "json",
		colNames: ["ID","集货区域编码","顺序","弹出口","物理仓","状态","批次号","已集入周转箱编码","周转箱集入时间"],
		colModel: [{name: "id", index: "id", hidden: true},		         
		           ...
		           {name: "plCode", index: "plCode", width: 120, resizable: true},
		           {name: "container", index: "container", width: 120, resizable: true},
		           {name: "startTime", index: "startTime", width: 120, resizable: true}
		           ],
		caption: '人工集货实时查询表',//集货点列表
	   	sortname: 'id',
	   	pager:"#pager",
	...
	$j("#saveWeight").click(function(){
		...
		if(null!=id&&""!=id){
			var result = loxia.syncXhrPost($j("body").attr("contextpath")+ "/json/updateShippingPointCollection.do", {"collectionCode" : collectionCode,"sort":sort,"popUpCode":popUpCode,"id":id});
			...
			var result = loxia.syncXhrPost($j("body").attr("contextpath")+ "/json/saveshippingpointcollection.do", {"collectionCode" : collectionCode,"sort":sort,"popUpCode":popUpCode,"physicalId":physicalId});
			if("success"==result["msg"]){
				jumbo.showMsg("保存成功");
				$j("#dialog_addWeight").dialog("close");
				$j("#t_wh_goods_collection_id").val("");
...
function editLpCodeWeight(obj){
	   ...
	   if (null != ids && ids.length > 0) {
			...
			var sort=$j(obj).parent().siblings().eq(4).text();
		    $j("#sort").val(sort);
			var popUpCode=$j(obj).parent().siblings().eq(5).text();
			$j("#popUpCode").val(popUpCode.replace(/(^\s*)|(\s*$)/g, ""));
			$j("#physicalId2").val($j(obj).parent().siblings().eq(6).text());
       }
