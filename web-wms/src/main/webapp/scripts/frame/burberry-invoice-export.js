...
$j(document).ready(function (){
	...
	$j("#export").click(function(){
		var id = $j("#sta_id").val();
		$j("#frmSoInvoice").attr("src",baseUrl + "/warehouse/exportburberryinvoice.do?staCmd.intStaType=21&staCmd.id=" + id);
	});
	...
	/* 这里编写必要的页面演示逻辑*/
		...
		$j("#export2").click(function(){
			...
			$j("#frmSoInvoice").attr("src",baseUrl + "/warehouse/exportburberryinvoice.do?staCmd.intStaType=42&staCmd.id=" + id);
		});
		...
		if(!plstatus.exception){
			$j("#tbl-dispatch-list").jqGrid({
				...
				datatype: "json",
				//["ID","配货批次号","状态","创建时间","最后核对时间","最后发货时间","开始配货时间"]
				colNames: ["ID","配货批次号",i18("INTSTATUS"),i18("CREATE_TIME"),i18("CHECKEDTIME"),i18("EXECUTEDTIME"),i18("PICKINGTIME")],
				colModel: [{name: "id", index: "id", hidden: true},
		...
		$j("#exportByPickingList").click(function(){
			$j("#frmSoInvoice").attr("src",baseUrl + "/warehouse/exportburberryinvoice.do?staCmd.pickingCode="+$j("#dphCode").text()+"&staCmd.pickingList.id=" + $j("#pickinglistid").val());
		});
});
