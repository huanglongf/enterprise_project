...
$j(document).ready(function (){
	...
	$j("#tbl-dispatch-list").jqGrid({
		url:$j("body").attr("contextpath") + "/findCompensationCheckList.json",
		postData:loxia._ajaxFormToObj("plForm"),
		datatype: "json",
		colNames: ["ID","索赔编码","相关订单号","索赔状态","索赔分类","索赔原因","店铺名称","物流商名称",
					"快递单号","承担方","是否外箱破损","产品包装是否破损","是否二次封箱","是否有产品退回","箱内填充是否充分","申请理赔总金额"
					,"物流部赔偿金额","快递商赔偿金额","发货仓","预警天数","平台备注","附件下载","订单发货时间","最后修改人","最后修改时间",
					"创建人","创建时间","OMS订单号","附加金额","附加金额备注","索赔金额确认状态","收件人姓名","收件人联系方式","收件人地址","处理意见"],
		colModel: [{name: "id", index: "co.id", hidden: true},
					...
					{name:"remark",index:"co.remark",width:90,resizable:true},
	               ...
	               {name:"claimAffirmStatus",index:"co.claimAffirmStatus",width:90,resizable:true,hidden: true},
		            {name:"receiver",index:"co.receiver",width:50},
		            {name:"mobile",index:"co.mobile",width:50},
		            {name:"receiverAddress",index:"co.receiverAddress",width:250},
					{name:"disposeRemark",index:"co.dispose_remark",width:90,resizable:true}
	               ],
		caption: '索赔单查询列表',
	   	sortname: 'co.id',
	    multiselect: false,
	...
	$j("#search").click(function(){
		$j("#tbl-dispatch-list").jqGrid('setGridParam',{url:loxia.getTimeUrl($j("body").attr("contextpath") + "/json/findCompensationCheckList.json"),
			postData:loxia._ajaxFormToObj("plForm")}).trigger("reloadGrid",[{page:1}]);
		jumbo.bindTableExportBtn("tbl-dispatch-list",{},loxia.getTimeUrl($j("body").attr("contextpath") + "/json/findCompensationCheckList.json"),loxia._ajaxFormToObj("plForm"));	
	});
	...
	$j("#dispose").click(function(){
		...
		var rs = loxia.syncXhrPost($j("body").attr("contextpath") + "/checkCompensate.json?logisticsepartmentAmt="
