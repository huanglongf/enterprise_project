...
$j(document).ready(function (){
	...
	$j("#needSn").click(function(){
		...
		$j("#search").click(function(){
			...
			$j("#tbl-inbound-purchase").jqGrid('setGridParam',{
				url:$j("body").attr("contextpath")+"/findpredefinedstabypagination.json?isVMI=true",postData:postData}).trigger("reloadGrid");
		});
	...
	if(!status.exception){
		...
		$j("#tbl-inbound-purchase").jqGrid({
			url:$j("body").attr("contextpath")+"/findpredefinedstabypagination.json?isVMI=true",
			datatype: "json",
			colNames: ["ID","STVID","STVTOTAL","STVMODE","相关单据号",i18("ENTITY_STA_CREATE_TIME"),
				...
				i18("ENTITY_STA_REMANENT"),i18("ENTITY_STA_COMMENT"),"isInboundInvoice","IS_PDA"],
			colModel: [
			           {name: "id", index: "id", hidden: true},		         
			         ...
			         //{name: "owner", index: "owner", width: 80, resizable: true,sortable:false},
			           {name: "shopId", index: "shopId", width: 80, resizable: true,sortable:false},
...
//	$j("#tbl-inbound-purchase").attr("style","width:1400px;");
			...
			loxia.asyncXhrPost($j("body").attr("contextpath") + "/purchasereceivestep1.json",
				...
				success:function (data) {
					...
					if(data&&data.SNmsg){
						...
						window.location=loxia.getTimeUrl($j("body").attr("contextpath")+"/warehouse/vmiOperationQuery.do");
					}else{
						jumbo.showMsg(i18("MSG_SUCCESS"));
						$j("#stvId").val(data.stvId);
						staToggle();
						var staData = $j("#staId").data("sta");
						//invoiceInfoInit(IS_INBOUND_INVOICE,"invoice1",$j("#po").text());
	...
	$j("#import").click(function(){
		...
		var url = $j("body").attr("contextpath") + "/warehouse/staimportforpurchase.do?sta.id="+$j("#staId").val()
		/*if(IS_INBOUND_INVOICE==1){
			if(IS_INVOICE_CODE == 0){
				jumbo.showMsg("请填写入正确的发票号！"+INVOICE_ERROR);
				return;
			} else {
				url += "&stv.invoiceNumber=" + $j("#invoice_invoiceNumber").val();
				url += "&stv.dutyPercentage=" + $j("#invoice_dutyPercentage").val();
				url += "&stv.miscFeePercentage=" + $j("#invoice_miscFeePercentage").val();
			}
		}*/
		if(errors.length>0){
			jumbo.showMsg(errors.join("<br/>"));
...
function executeInventory(closeSta,stvId,staId){
	if(validateLocationQuantity()){
		...
		/*if(IS_INBOUND_INVOICE==1){
			if(IS_INVOICE_CODE == 0){
				jumbo.showMsg("请填写入正确的发票号！");
				return;
			}
			postData["stv.invoiceNumber"] = $j("#invoice1_invoiceNumber").val();
			postData["stv.dutyPercentage"] = $j("#invoice1_dutyPercentage").val();
			postData["stv.miscFeePercentage"] = $j("#invoice1_miscFeePercentage").val();
		}*/
		...
		loxia.asyncXhrPost(window.parent.$j("body").attr("contextpath")+"/json/executeinventory.json",postData,
		{
			success:function (data) {
				if(data && data.result != 'success'){
					...
					window.location=$j("body").attr("contextPath")+"/warehouse/vmiOperationQuery.do";
				}	
			}, 
	       error:function(){
				 jumbo.showMsg(i18("MSG_FAILURE"));	
