...
function printSingleDelivery(tag,event){
	...
	var url = $j("body").attr("contextpath") + "/printPickinglistReturn.json?plCmd.id=" + plid + "&plCmd.staId=" + staid;
...
$j(document).ready(function (){
	...
	$j("#tbl-pickingListDetail").jqGrid({
		...
		colModel: [{name: "id", index: "id", hidden: true},
	               ...
	               {name: "idForBtn", width: 150,sortable: false,resizable:true, formatter:"buttonFmatter", formatoptions:{"buttons":{label:i18("PRINGT"), onclick:"printSingleDelivery(this,event);"}}}
	               ],
	...
	$j("#printPickingList").click(function(){
		...
		var plid = $j("#div2").attr("plId");
		jumbo.showMsg(i18("TRUNKPACKINGLISTINFO"));
		var url = $j("body").attr("contextpath") + "/printPickinglistReturn.json?plCmd.id=" + plid;
		//var rs = loxia.syncXhrPost(url);
		printBZ(loxia.encodeUrl(url),true);				
		loxia.unlockPage();
	});
