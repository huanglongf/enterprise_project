...
$j(document).ready(function (){
	...
	$j("#search").click(function(){
		...
		$j("#tbl-inbound-purchase").jqGrid('setGridParam',{
			url:$j("body").attr("contextpath")+"/findpredefinedstabypagination.json?sample=true",postData:postData}).trigger("reloadGrid");
	});
	...
	if(!status.exception){
		...
		$j("#tbl-inbound-purchase").jqGrid({
			url:$j("body").attr("contextpath")+"/findpredefinedstabypagination.json?sample=true",
			datatype: "json",
	...
	$j("#import").click(function(){
		...
		var url = $j("body").attr("contextpath") + "/warehouse/staimportforrepair.do?sta.id="+$j("#staId").val();
		if(errors.length>0){
...
function importReturn(){
	window.location=loxia.getTimeUrl($j("body").attr("contextpath")+"/warehouse/onceLibrary.do");
}
