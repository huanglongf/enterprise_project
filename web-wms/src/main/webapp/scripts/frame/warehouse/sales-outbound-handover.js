...
function cancelDetial(tag,event){
	...
	if(result && result.result == "success"){
		...
		showErrorInfoDialog(i18("CANCEL_FAILED"));//作废失败
	}
}
...
$j(document).ready(function (){
	...
	$j("#import").click(function(){
		...
		if(errors.length > 0){
			showErrorInfoDialog(errors.join("<br />&nbsp; &nbsp; &nbsp;"));
			return;
		}
...
//					{name: "idForBtn", width: 120,resizable:true, formatter:"buttonFmatter", formatoptions:{"buttons":{label:i18("CANCEL"), onclick:"cancelDetial(this,event);"}}}],//作废
	...
	$j("#createOrder").click(function(){
		...
		if(result && result.result == "success"){
			...
			showErrorInfoDialog(i18("LIST_CREATE_FAILED"));//交货清单创建失败
		}
	});
	...
	$j("#btnDetialHo").click(function(){
		...
		if(result && result.result == "success"){
			...
			showErrorInfoDialog(i18("FAILED"));//交接失败
		}
	});
	...
	$j("#btnDetialPrint").click(function(){
		...
		if(rs && rs.result == 'success'){
			...
			showErrorInfoDialog(rs.message);
		}
	}); 
	$j("#createOrderByHand").click(function(){
		if (!packageIds.length  >0){
			showErrorInfoDialog("数据为空，操作异常");
			return ;
		}
...
//	    }
		...
		showErrorInfoDialog("操作异常");
		 i++;
	}
	 if(i>0){
		 showErrorInfoDialog(msg);
	 }
	 return i;
}
//作废交接清单明细行
