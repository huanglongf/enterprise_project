...
function showDetail(tag){
	...
	if(rs && rs["pickingList"]){
		...
		sortingMode = rs["pickingList"]["sortingModeInt"]; // 拣货模式 查询用
		toLocation = rs["pickingList"]["toLocation"];
		checkMode = rs["pickingList"]["intCheckMode"];
		var isShowButton = rs["pickingList"]["packingType"];
		if(isShowButton == 1){
			$j("#printPickingListThree").removeClass("hidden");
		}else{
			$j("#printPickingListThree").addClass("hidden");
		}
	}
...
$j(document).ready(function (){
	...
	$j("#tbl-waittingList").jqGrid({
		...
		colModel: [
		           ...
		           {name:"isInvoice",index:"isInvoice",width:80,resizable:true,formatter:'select',editoptions:trueOrFalse},
	               {name:"city",index:"city",width:80,resizable:true},
	               {name:"categoryName",index:"categoryName",width:80},
	               {name:"skuSizeName",index:"skuSizeName",width:80},
	               {name:"isPreSale",index:"isPreSale",width:80},
	               {name:"worker",index:"worker",width:100}
	               ],
		...
		loadComplete: function(){
			...
			// $j("#batch_pickZoneCode option:gt(0)").remove();
			zoonResult = result;
			$j("#batch_pickZoneCode option:not(:first)").remove();
			for(var idx in result){
					$j("<option value='" + result[idx].id + "'>"+ result[idx].name +"</option>").appendTo($j("#batch_pickZoneCode"));
			}
	...
	$j("#back,#back2,#back3").click(function(){
		...
		$j("#pickZoneCode").find("option").remove();
		$j("#pickZoneCode").append( "<option value=\"\">--请选择区域--</option>" );
		zoonResult = "";
...
//			if(""==pickZoneId){pickZoneId=null;}
			if(!skusIdAndQty){
				skusIdAndQty = "";
			}
			jumbo.showMsg(i18("OPERATING"));// 配货清单打印中，请等待...
			if(zoonResult.length == 0 || pickZoneId != ""){
				var url = $j("body").attr("contextpath") + "/printpickinglistmode1.json?rt="+ dt +"&plCmd.id="+ plid+"&plCmd.pickZoneId="+pickZoneId;  
				url += "&plCmd.code=" + plcode + "&plCmd.planBillCount=" + plcount + "&plCmd.packageSku=" +skusIdAndQty;
				url += "&plCmd.planSkuQty=" + plskuqty + "&plCmd.operator.userName=" + ploper;
				printBZ(loxia.encodeUrl(url),true);
			}else{
				// 如果没有选择 区域，则循环当前批次下的区域，打印多张
				for(var idx in zoonResult){
					var pickZoneId =zoonResult[idx].id;
					var url = $j("body").attr("contextpath") + "/printpickinglistmode1.json?rt="+ dt +"&plCmd.id="+ plid+"&plCmd.pickZoneId="+pickZoneId;  
					url += "&plCmd.code=" + plcode + "&plCmd.planBillCount=" + plcount + "&plCmd.packageSku=" +skusIdAndQty;
					url += "&plCmd.planSkuQty=" + plskuqty + "&plCmd.operator.userName=" + ploper;
					printBZ(loxia.encodeUrl(url),true);
	...
	$j("#printPickingListNew").click(function(){
		...
		if(beforePrintValidate()){
			...
			jumbo.showMsg(i18("OPERATING"));// 配货清单打印中，请等待...
			if(zoonResult.length == 0 || pickZoneId != ""){
				var url = $j("body").attr("contextpath") + "/printpickinglistnewmode1.json?rt="+ dt +"&pickingListId="+ plid+"&pickZoneId="+pickZoneId;
				printBZ(loxia.encodeUrl(url),true);
			}else{
				// 如果没有选择 区域，则循环当前批次下的区域，打印多张
				for(var idx in zoonResult){
	...
	//打印拣货单模式三 礼盒包装
	$j("#printPickingListThree").click(function(){
		loxia.lockPage();
		if(beforePrintValidate()){
			var dt = new Date().getTime();
			var plid = $j("#div2").attr("plId");
...
//			if(""==pickZoneId){pickZoneId=null;}
			...
			var url = $j("body").attr("contextpath") + "/printpickinglistmode1.json?rt="+ dt +"&plCmd.id="+ plid+"&plCmd.pickZoneId="+pickZoneId;  
			url += "&plCmd.code=" + plcode + "&plCmd.planBillCount=" + plcount + "&plCmd.packageSku=" +skusIdAndQty;
			url += "&plCmd.planSkuQty=" + plskuqty + "&plCmd.operator.userName=" + ploper+ "&plCmd.packingType=1";
			printBZ(loxia.encodeUrl(url),true);
		}
		loxia.unlockPage();
	});
	...
	$j("#printTrunkPackingList").click(function(){
		...
		if(beforePrintValidate()){
			var dt = new Date().getTime();
			var plid = $j("#div2").attr("plId");
			var rs = loxia.syncXhrPost($j("body").attr("contextpath") + "/queryDispatchListOutputCount.json?rt="+ dt,{"plCmd.id":plid});
...
//				alert(plid+" "+plcode);
				...
				var url = $j("body").attr("contextpath") + "/printtrunkpackinglistmode1Out.json?rt="+ dt +"&plCmd.id=" + plid + "&plCmd.code=" + plcode;
...
//		}else{
			...
			if(beforePrintValidate()){
				var dt = new Date().getTime();
				var plid = $j("#div2").attr("plId");
				var url = $j("body").attr("contextpath") + "/printdeliveryinfomode1.json?rt="+ dt +"&plCmd.id=" + plid;
				var pl = loxia.syncXhrPost($j("body").attr("contextpath") + "/getPickingList.json?rt="+ dt,{"pickingListId":plid});
				...
				if(true == isPostpositionExpressBill){
					$j("#postPrintInfo").removeClass("hidden");
...
//			}
	...
	$j("#btnSoInvoice").click(function(){
		var id = $j("#div2").attr("plId").toString();
		$j("#frmSoInvoice").attr("src",$j("body").attr("contextpath") + "/warehouse/exportSoInvoice.do?pickingListIds=" + id);
	});
	...
	$j("#btn-query").click(function(){
		...
		/*var isPreSale=$j("#isPreSale").val();
		postData["isPreSale"]=isPreSale;*/
		var priority=postData["priority"];
		postData["priority"]=priority.join(",");
		var priority=postData["pickingList.city"];
		postData["pickingList.city"]=priority.join(",");
	 ...
	 * 批量 税控发票导出 
	 */
	$j("#batch_btnSoInvoice").click(function(){
		var ids = $j("#tbl-waittingList").jqGrid('getGridParam','selarrrow');
		if(null == ids && ids.length == 0) {
			jumbo.showMsg("请勾选需要操作数据,谢谢!~"); 
			return false;
		}
		$j("#frmSoInvoice").attr("src",$j("body").attr("contextpath") + "/warehouse/exportSoInvoice.do?pickingListIds=" + ids);
	});
	
	
	
	/**
	 * (待配货清单列表页面)批量 打印拣货单模式一
	 */
	$j("#batch_printPickingList").click(function(){
		var ids = $j("#tbl-waittingList").jqGrid('getGridParam','selarrrow');
		if (null != ids && ids.length > 0) {
			var rs = loxia.syncXhrPost($j("body").attr("contextpath") + "/json/batchcheckforstastatus.json?pickingListIds=" + ids);
			if (rs && rs.msg) {
				//loxia.lockPage();
				var dt = new Date().getTime();
				var pickZoneId=$j("#batch_pickZoneCode").val();
					if(zoonResult.length == 0 || pickZoneId != ""){
						var url = $j("body").attr("contextpath") + "/printPickingListModeBulk1.json?rt="+ dt +"&plList="+ ids+"&pickZoneId="+pickZoneId;
						$j("#printerChoose").attr("url",url);
						//$j("#printerChoose").dialog("open");
						//printBZ(loxia.encodeUrl(url),true);
						...
						// 如果没有选择 区域，则循环当前批次下的区域，打印多张
						for(var idx in zoonResult){
							arr.push(zoonResult[idx].id);
						}
						var pickingZoneIds = arr.join(",");
						var url = $j("body").attr("contextpath") + "/printPickingListModeBulk1.json?rt="+ dt +"&plList="+ ids+"&pickingZoneIds="+pickingZoneIds;
						$j("#printerChoose").attr("url",url);
						//$j("#printerChoose").dialog("open");
						//printBZ(loxia.encodeUrl(url),true);
					}
					...
					$j("#printerChoose").dialog("open");
					//loxia.unlockPage();
			}else if(rs && rs.msg == 'data_error'){
				jumbo.showMsg("数据为空，数据出错..."); 
			}else {
	...
	/**
	 * (待配货清单列表页面)批量 打印拣货单模式二
	 */
	$j("#batch_printPickingListNew2").click(function(){
		var ids = $j("#tbl-waittingList").jqGrid('getGridParam','selarrrow');
		if (null != ids && ids.length > 0) {
			var rs = loxia.syncXhrPost($j("body").attr("contextpath") + "/json/batchcheckforstastatus.json?pickingListIds=" + ids);
			if (rs && rs.msg) {
				//loxia.lockPage();
					...
					var pickZoneId=$j("#batch_pickZoneCode").val();
					if(zoonResult.length == 0 || pickZoneId != ""){
						var url = $j("body").attr("contextpath") + "/printPickingListModeBulkNew2.json?rt="+ dt +"&plList="+ ids+"&pickZoneId="+pickZoneId;
						$j("#printerChoose").attr("url",url);
						//$j("#printerChoose").dialog("open");
					}else{
						var arr = [];
						...
						var pickingZoneIds = arr.join(",");
						var url = $j("body").attr("contextpath") + "/printPickingListModeBulkNew2.json?rt="+ dt +"&plList="+ ids+"&pickingZoneIds="+pickingZoneIds;
						$j("#printerChoose").attr("url",url);
					}
					$j("#printerChoose").dialog("open");
				//loxia.unlockPage();
	...
	/**
	 * (待配货清单列表页面)批量 打印装箱清单按钮功能
	 */
	$j("#batch_printTrunkPackingList").click(function(){
		var ids = $j("#tbl-waittingList").jqGrid('getGridParam','selarrrow');
		if (null != ids && ids.length > 0) {
				var rs = loxia.syncXhrPost($j("body").attr("contextpath") + "/json/batchqueryofoutputcount.json?pickingListIds="+ ids);
				if(rs && rs["msg"] == 'success' && parseInt(rs["count"]) == 0){
					loxia.lockPage();
					var dt = new Date().getTime();
					var url = $j("body").attr("contextpath") + "/printTrunkPackingListMode1OutBulk.json?rt="+ dt +"&plList=" + ids;
					$j("#printerChoose").attr("url",url);
					$j("#printerChoose").dialog("open");
	 ...
	 * (待配货清单列表页面)批量 打印完成按钮
	 */
	$j("#batch_printDone").click(function(){
		var ids = $j("#tbl-waittingList").jqGrid('getGridParam','selarrrow');
		if (null != ids && ids.length > 0) {
			if(confirm("确认已全部完成该批次打印工作？")){
				var rs = loxia.syncXhrPost($j("body").attr("contextpath") + "/batchupdatepicklistwhstatus.json?pickingListIds=" + ids);
				if(rs && rs["msg"] == 'SUCCESS'){
					jumbo.showMsg("打印完成操作成功！");
					var postData = loxia._ajaxFormToObj("query-form");
					$j("#tbl-waittingList").jqGrid('setGridParam',{
	 ...
	 * 开始打印
	 */
	$j("#batch_printStart").click(function(){
		var ids = $j("#tbl-waittingList").jqGrid('getGridParam','selarrrow');
		if (null != ids && ids.length > 0) {
			var rs = loxia.syncXhrPost($j("body").attr("contextpath") + "/startPrintPicklist.json?pickingListIds=" + ids);
			if(rs && rs["msg"] == 'SUCCESS'){
				jumbo.showMsg("操作成功！");
	 ...
	 * 批量 打印物流面单
	 */
	$j("#batch_printDeliveryInfo").click(function(){
		var ids = $j("#tbl-waittingList").jqGrid('getGridParam','selarrrow');
		if (null != ids && ids.length > 0) {
			loxia.lockPage();
			var dt = new Date().getTime();
			var url = $j("body").attr("contextpath") + "/printdeliveryinfomode1Bulk.json?rt="+ dt +"&plList=" + ids;
			$j("#printerChoose").attr("url",url);
			$j("#printerChoose").dialog("open");
...
function showDetail1(pCode){
	...
	if(pCode.length==0){
		...
		if(rs&&rs["pickingList"]){
			//加载区域下拉框
			var result = loxia.syncXhr(window.$j("body").attr("contextpath")+"/json/getPickingDistrict.do",{"pickingListId":rs["pickingList"]["id"]});
			zoonResult = result;
			for(var idx in result){
				if(result[idx].id == null){
					result[idx].id = 0;
