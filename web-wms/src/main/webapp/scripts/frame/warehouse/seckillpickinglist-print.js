...
$j(document).ready(function (){
	...
	$j("#tbl-waittingList").jqGrid({
		...
		datatype: "json",
		colNames: ["ID","outputCount", i18("BATCHNO"),i18("CREATETIME"),i18("PLANBILLCOUNT"),i18("PLAYSKUQTY"),i18("INTSTATUS"),"物流"],
		colModel: [{name: "id", index: "id", hidden: true},
	...
	$j("#occupiedInv,#tryTrans").click(function(){
		...
		if(rs){
			...
			$j("#tbl-waittingList").jqGrid('setGridParam',{url:$j("body").attr("contextpath") + "/getallpickinglist.json",postData:{"pickingList.checkMode":"PICKING_SECKILL"}}).trigger("reloadGrid");
...
function showDetail1(pCode){
	if(pCode.length==0){
		...
		var rs = loxia.syncXhrPost($j("body").attr("contextpath") + "/json/getpickinglistbycode.json",{"pickingList.code":pCode,"pickingList.checkMode":"PICKING_SECKILL"});
		if(rs&&rs["pickingList"]){
