...
$j(document).ready(function(){
	...
	//O2O门店
	$j("#btnSearchO2O").click(function(){
		$j("#O2OQueryDialog").dialog("open");
	});
	...
	var ruleNameResult = loxia.syncXhrPost(window.$j("body").attr("contextpath")+"/json/getAllRuleName.do?modeName="+"多件");
...
//	}
	...
	$j("#tbl-staList-query").jqGrid({
		datatype: "json",
		colNames: ["ID",i18("CODE"),i18("REFSLIPCODE"),i18("SHOPID"),i18("LPCODE"),i18("STATUS"),i18("CREATETIME"),"平台订单时间",i18("ISNEEDINVOICE"),i18("STVTOTAL"),"是否包含SN号商品","是否特殊处理","商品数量","是否QS","包装类型"],
		colModel: [{name: "id", index: "id", hidden: true},		         
		           ...
		           {name: "stvTotal", index: "stvTotal", width: 90, resizable: true, hidden: true},
		           ...
		           {name: "stvTotal", index: "stvTotal", width: 120, resizable: true, hidden: true},
...
//		postData["isGroupSku"]=false;
		...
		if($j("#CountAreasCp").attr("checked")){
			postData["countAreasCp"]=true;
		}else{
			postData["countAreasCp"]=false;
		}
		if($j("#whCountAreas").attr("checked")){
			postData["mergeWhZoon"]=1;
		}else{
			postData["mergeWhZoon"]=0;
		}
		//O2O全选
...
//		alert(array);
	...
	$j("#reset").click(function(){
		...
		$j("#CountAreas").attr("checked",false);
		$j("#whCountArea").attr("checked",false);
		$j("#whCountAreas").attr("checked",false);
		$j("#priorityCity").multipleSelect("uncheckAll");
		$j('#priorityCity').multipleSelect('refresh');
		$j("#priorityId").multipleSelect("uncheckAll");
		$j('#priorityId').multipleSelect('refresh');
...
//				if($j("#issingle").attr("checked")==true){
//					postData1["sta.skuQty"]="";
//				}else{
//					postData1["sta.skuQty"]=2;
//				}
//				postData1["sta.isSedKill"]=false;
//				postData1["isGroupSku"]=false;
				postData1["sta.pickingType"]="SKU_MANY";
				var clist = $j("#skuConfigtd input");
				for(var i=0;i<clist.length;i++){
	...
	$j("#autoConfirm").click(function(){
		...
		var skuQty = $j("#sumCount").val();
		var skuCodes = "";
		//判断作业单商品总数是否超过数量总数
		for(var i = 0 ; i < ids.length ; i++){
			var skuTotal= $j("#tbl-staList-query").getCell(ids[i],"stvTotal");
			if (+skuTotal > +skuQty){
				skuCodes += "["+$j("#tbl-staList-query").getCell(ids[i],"code")+"]";
				
			}
		}
		if (skuCodes != ""){
			if(!confirm("作业单"+skuCodes+"超过商品总数上限，是否继续创建，继续将剔除该作业单!")){
				return;
			}
		}
