...
function loadOrderDetail(url){
	...
	$j("#tbl-orderDetail").jqGrid({
		...
		colNames: ["ID","序列","status",i18("CODE"),i18("INTSTATUS"),i18("REFSLIPCODE"),i18("INTTYPE"),i18("SHOPID"),"物流服务",i18("CREATETIME"),"计划总件数",i18("OPETION"),i18("OPETION")],
		colModel: [{name: "id", index: "id", hidden: true},
	               ...
	               {name: "idForBtn", width: 100,resizable:true,sortable: false, formatter:"buttonFmatter", formatoptions:{"buttons":{label:i18("PRINGT"), onclick:"printSingleDelivery(this,event);"}}},
	               {name: "idForMaCaoHGDBtn", width: 100,resizable:true,sortable: false, formatter:"buttonFmatter", formatoptions:{"buttons":{label:i18("MACAOPRINGT"), onclick:"printSingleMaCaoHGD(this,event);"}}}],
...
//澳门件海关单单个打印
function printSingleMaCaoHGD(tag,event){
	var id = $j(tag).parents("tr").attr("id");
	//判断是否可以打印海关单
	var rs = loxia.syncXhrPost($j("body").attr("contextpath") + "/json/checkIsAllowPrintMacaoHGD.json",{"staId":id});
	if(rs && !rs["result"]){
		jumbo.showMsg("非澳门件或者澳门件订单金额未达到设定值，不允许打印海关单！");
		return;
	}
	var url = $j("body").attr("contextpath") + "/printImportMacaoHGDEntryList.json?staId="+id;
	printBZ(loxia.encodeUrl(url),true);
	loxia.unlockPage();
} 
...
$j(document).ready(function (){
	...
	$j("#tbl-waittingList").jqGrid({
		url:$j("body").attr("contextpath") + "/getallpickinglist.json",
		datatype: "json",
		...
		colModel: [
		           ...
		           {name: "planBillCount", index: "planBillCount", width: 70, resizable: true},
		           {name: "planSkuQty", index: "planSkuQty", width: 70, resizable: true},
	               ...
	               {name:"isPreSale",index:"isPreSale",width:80,resizable:true},
	...
	$j("#printPickingList").click(function(){
		...
		if(beforePrintValidate()){
			...
			if(zoonResult.length == 0 || pickZoneId != ""){
				var url = $j("body").attr("contextpath") + "/printpickinglistmode1.json?rt="+ dt +"&plCmd.id="+ plid+"&plCmd.pickZoneId="+pickZoneId+"&flag=all";  
	...
	$j("#printPickingListNew").click(function(){
		...
		if(beforePrintValidate()){
			...
			jumbo.showMsg(i18("OPERATING"));// 配货清单打印中，请等待...
			if(zoonResult.length == 0 || pickZoneId != ""){
				var url = $j("body").attr("contextpath") + "/printpickinglistnewmode1.json?rt="+ dt +"&pickingListId="+ plid+"&pickZoneId="+pickZoneId+"&flag=all";
				printBZ(loxia.encodeUrl(url),true);
			}else{
				// 如果没有选择 区域，则循环当前批次下的区域，打印多张
				for(var idx in zoonResult){
...
//		}
	...
	$j("#importEntry").click(function(){
			var plid = $j("#div2").attr("plId");
			loxia.lockPage();
			var url = $j("body").attr("contextpath") + "/printImportEntryList.json?pickinglistId="+plid;
			printBZ(loxia.encodeUrl(url),true);
			loxia.unlockPage();
	});
	...
	$j("#importMacaoEntry").click(function(){
		var plid = $j("#div2").attr("plId");
		loxia.lockPage();
		var url = $j("body").attr("contextpath") + "/printImportMacaoEntryList.json?pickinglistId="+plid;
		printBZ(loxia.encodeUrl(url),true);
		loxia.unlockPage();
    });
	
	//打印澳门海关单
	$j("#importMacaoHGDEntry").click(function(){
		var plid = $j("#div2").attr("plId");
		//判断是否可以打印海关单
		var rs = loxia.syncXhrPost($j("body").attr("contextpath") + "/json/checkIsAllowPrintMacaoHGD.json",{"pickinglistId":plid});
		if(rs && !rs["result"]){
			jumbo.showMsg("非澳门件或者澳门件订单金额未达到设定值，不允许打印海关单！");
			return;
		}
		loxia.lockPage();
		var url = $j("body").attr("contextpath") + "/printImportMacaoHGDEntryList.json?pickinglistId="+plid;
		printBZ(loxia.encodeUrl(url),true);
		loxia.unlockPage();
	});
	...
	$j("#btn-query").click(function(){
		...
		$j("#tbl-waittingList").jqGrid('setGridParam',{
			url:window.$j("body").attr("contextpath")+"/json/getallpickinglist.do",
			postData:postData,
			page:1
		}).trigger("reloadGrid");
	});
