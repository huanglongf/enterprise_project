...
$j(document).ready(function(){
	...
	$j("#showSkuDialog").dialog({title: "秒杀商品查询", modal:true, autoOpen: false, width: 730});
	$j("#tbl_sku_query_dialog").jqGrid({
		//url:$j("body").attr("contextpath") + "/queryGroupBuyingSku.json",
		url:$j("body").attr("contextpath") + "/selectallseckillsku.json",
		datatype: "json",
		colNames: ["ID","SKUS","secKillString",i18("TITLENAME"),i18("KINDS")],
		colModel: [ 
		           {name: "id", index: "id", hidden: true},
		           {name: "skus", index:"skus", hidden:true},
		           {name: "secKillString", index:"secKillString",hidden:true},
				   {name: "skusString", index: "skusString", width: 600, resizable: true},
		           {name: "kind", index: "kind",width: 60, resizable: true},
		           ],
		caption: "秒杀商品列表",
	...
	$j("#btnSkuSelectConfirm").click(function(){
		...
		$j("#skuName").val(data["secKillString"]);
		$j("#skus").val(data["skus"]);
...
//	}
	...
	$j("#btn-query").click(function(){
		...
		postData["sta.pickingType"]="SECKILL";
	...
	$j("#autoConfirm").click(function(){
		...
		var arrCity = postData["city"];
		postData["city"] = arrCity.join(",");
		postData["transTimeType"] = $j("#transTimeType").val();
		...
		if(result&&result.result){//成功后更新原始查询条件列表
			...
			var arrCity = postData1["city"];
			postData1["city"] = arrCity.join(",");
			postData1["shoplist"] = $j("#postshopInput").val();
			//postData1["sta.isSedKill"]=true;
			postData1["sta.pickingType"]="SECKILL";
