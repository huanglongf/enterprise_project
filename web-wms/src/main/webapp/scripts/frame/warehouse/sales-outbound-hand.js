...
function addBarCodeFanht(){
	...
	if($j("#p1").attr("data")=="true" && $j("#isHaoCai").val() !="0" ){
...
//	loxia.byId("autoWeigth").setEnable(false);
	...
	$j("#billsId").keydown(function(event){
		if(event.keyCode == 13){
			...
			if(rs && rs.result=="success"){
				...
				$j("#isHaoCai").val(rs.sta["isHaoCai"]);//耗材
				$j("#shopId").html(rs.sta["shopId"]);
				$j("#staDiv").attr("staId",rs.sta["id"]);
				if(rs.sta["tansPgId"]!=null){
					$j("#staDiv").attr("isOtwoO",true);
				}else{
					$j("#staDiv").attr("isOtwoO",false);
				}
				if(rs.sta["isPreSale"]=="1"){
	...
	$j("#executeOutbound").click(function(){
			...
			postData["isOtwoO"]=$j("#staDiv").attr("isOtwoO");
