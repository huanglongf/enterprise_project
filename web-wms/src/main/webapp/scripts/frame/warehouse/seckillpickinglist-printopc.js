...
function printSingleDelivery(tag,event){
	...
	var whid = $j(tag).parents("tr").children("td:eq(12)").attr("title");
...
//根据运营中心仓库查询
	...
	$j("#tbl-waittingList").jqGrid({
		...
		datatype: "json",
		colNames: ["ID","outputCount",i18("BATCHNO"),i18("CREATETIME"),i18("PLANBILLCOUNT"),i18("PLAYSKUQTY"),i18("INTSTATUS"),"物流","所属仓库","仓库id"],
		colModel: [{name: "id", index: "id", hidden: true},
...
//		}
	...
	$j("#occupiedInv,#tryTrans").click(function(){
		...
		if(rs){
			...
			$j("#tbl-waittingList").jqGrid('setGridParam',{url:$j("body").attr("contextpath") + "/getallpickinglistbywid.json",postData:{"pickingList.checkMode":"PICKING_SECKILL"}}).trigger("reloadGrid");
	...
	$j("#printDeliveryInfo").click(function(){
		...
		if(isEmsOlOrder == 'true' && (ems == 'EMS' || ems == 'EMSCOD')){
