...
$j(document).ready(function(){
	...
	var ruleNameResult = loxia.syncXhrPost(window.$j("body").attr("contextpath")+"/json/getAllRuleName.do?modeName="+"团购");
	...
	$j("#btn-query").click(function(){
		...
		postData["zoonId"]=$j("#zoonCode1").val();
//		postData["sta.isSedKill"]=false;
//		postData["isGroupSku"]=true;
		postData["sta.pickingType"]="GROUP";
	...
	$j('#confirm').click(function(){
		...
		if(staIds.length == 1){
			...
			postData['pickingList.checkMode']="PICKING_GROUP";
...
//				postData1["isGroupSku"]=true;
				postData1["sta.pickingType"]="GROUP";
				var clist = $j("#skuConfigtd input");
				for(var i=0;i<clist.length;i++){
		...
		}else if (staIds.length > 1){
			jumbo.showMsg("手动创团购单只允许勾选1行，选择多行请用自动创建功能！");//请选择作业申请单
		}else{	
...
//			postData1["sta.isSedKill"]=false;//标识非秒杀订单
//			postData1["isGroupSku"]=true;//标识要查询团购订单
			postData1["sta.pickingType"]="GROUP";
			//商品大小限制条件数据封装
			var clist = $j("#skuConfigtd input");
			for(var i=0;i<clist.length;i++){
        ...
        if(null!=result&&null!=result["result"]){
        	...
        	if(document.getElementById("priorityId").options.length>0){
        		if(null!=result["result"]["priority"]&&""!=result["result"]["priority"]){
        			var arrayCity={};
        			if(result["result"]["priority"].indexOf("opposite")>0){
        				arrayCity=result["result"]["priority"];
        			}else{
