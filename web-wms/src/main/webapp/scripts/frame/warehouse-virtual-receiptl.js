...
$j(document).ready(function (){
	...
	$j("#exportZip").click(function(){
		var id = $j("#sta_id").val();
		var code = $j("#sta_code").html();
		$j("#upload").attr("src",$j("body").attr("contextpath") + "/warehouse/exportInboundShelves.do?sta.id=" + id + "&sta.code=" + code);	
	});
	...
	$j("#import").click(function(){
		...
		if(errors.length>0){
			...
			$j("#importForm").attr("action",
				loxia.getTimeUrl($j("body").attr("contextpath") + "/warehouse/importStaInboundShelves.do?sta.id="+$j("#sta_id").val()));
...
//		loxia.asyncXhrPost(window.parent.$j("body").attr("contextpath")+ "/json/inBoundAffirm.json",
//				{'sta.id':$j("#sta_id").val()},
//				{
