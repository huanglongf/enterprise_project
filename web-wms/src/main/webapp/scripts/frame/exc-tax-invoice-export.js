...
/* 这里编写必要的页面演示逻辑*/
	...
	$j("#tbl-orderList").jqGrid({
		...
		gridComplete: function(){
			...
			for(var i=0;i < ids.length;i++){
				...
				$j("#tbl-orderList").jqGrid('setRowData',ids[i],{"intType":type});
	...
	$j("#search").click(function (){
		$j("#tbl-orderList").jqGrid('setGridParam',{
			url:loxia.getTimeUrl(baseUrl + "/queryExcStaExport.json"),postData:loxia._ajaxFormToObj("queryForm")
			}).trigger("reloadGrid");
	});
	...
	$j("#export").click(function(){
		var id = $j("#sta_id").val();
		$j("#frmSoInvoice").attr("src",baseUrl + "/warehouse/exportExcStaSoInvoice.do?staCmd.id=" + id);
		$j("#export").addClass("hidden");
	});
});
