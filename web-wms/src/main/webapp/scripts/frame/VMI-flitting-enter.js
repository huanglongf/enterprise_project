...
$j.extend(loxia.regional['zh-CN'],{
	...
	MSG_CONFIRM_CLOSESTA		:	"您确定要关闭本次VMI调拨入库收货吗?",
	...
	TABLE_CAPTION_STA			:	"VMI调拨入库待收货列表",
	TABLE_CAPTION_STALINE		:	"VMI调拨入库待收货明细表",
...
//	 	$j("#tbl-shelve").addClass("hidden");
		$j("#tbl-orderNumber").jqGrid('setGridParam',
			{url:$j("body").attr("contextpath")+"/queryVMIFlittingEnterLine.json?sta.id="+staId}).trigger("reloadGrid",[{page:1}]);
		$j("#createTime").html(sta.createTime);
		$j("#staCode").html(sta.code);
		$j("#status").html(sta.invStrutsName);
		$j("#mainName").html(sta.mainName);
		$j("#ownerOuName").html(sta.ownerOuName);
		$j("#addiOwnerOuName").html(sta.addiOwnerOuName);
	}else{
...
$j(document).ready(function (){
	...
	var entry=$j("body").attr("contextpath")+"/warehouse/initToVMIFlittingEnter.do";
	...
	if(!status.exception){
		...
		$j("#tbl-inbound-purchase").jqGrid({
			url:$j("body").attr("contextpath")+"/stalistjsonnopagination.json?sta.intType=90",
			datatype: "json",
			colNames: ["ID","STVID","STVTOTAL","STVMODE",i18("ENTITY_STA_CREATE_TIME"),i18("ENTITY_STA_CODE"),
				i18("ENTITY_STA_PROCESS"),i18("ENTITY_STA_UPDATE_TIME"),i18("ENTITY_STA_PO"),"目标店铺","原始店铺","库存状态","操作人"],
			colModel: [
			           {name: "id", index: "id", hidden: true},		         
			           ...
			           {name: "mainName", index: "mainName", width: 120, resizable: true,sortable:false},
		               ...
		               {name: "creater",index:"creater",width:150,resizable:true,sortable:false}],
			caption: i18("TABLE_CAPTION_STA"),
			postData:{"columns":"id,stvId,stvTotal,stvMode,createTime,code,processStatus,inboundTime,mainName,ownerOuName,addiOwnerOuName,invStrutsName,creater"},
	...
	$j("#tbl-orderNumber").jqGrid({
		...
		colNames: ["ID","STAID","SKUID","OWNER","skuCost",i18("ENTITY_SKU_BARCODE"),i18("ENTITY_SKU_JMCODE"),i18("ENTITY_SKU_KEYPROP"),
			i18("ENTITY_SKU_NAME"),i18("ENTITY_SKU_SUPCODE"),i18("ENTITY_STALINE_TOTAL"),"completeQuantity",
			i18("ENTITY_STALINE_CURRENT"),
			i18("ENTITY_STALINE_SNS")],
		colModel: [
		           ...
		           {name: "completeQuantity", index: "completeQuantity", width: 100, hidden: true,sortable:false},
...
function getSkuInfo(line){	 
	var html=[];
	html.push("<tr skuId=\""+line.skuId+"\" staLineId=\""+line.stalineId+"\" total=\""+line.totalQuantity+"\"  owner=\""+line.owner+"\" batchCode='"+line.batchCode+"' >");
...
function getLocationsTable(line){
	...
	$j.each(line.stvLines,function(i, item) {
		...
		tb.push("<tr id=\""+item.id+"\" skuId=\""+line.skuId+"\" staLineId=\""+line.stalineId+"\"  owner=\""+line.owner+"\" batchCode='"+line.batchCode+"' >");
...
function getLocationsTableTemplete(line){
	  ...
	  var url=$j("body").attr("contextpath") + "/findallavaillocations.json";
	  var tb2= " <tbody> "+
					"<tr  skuId=\""+line.skuId+"\" staLineId=\""+line.stalineId+"\"  owner=\""+line.owner+"\" batchCode='"+line.batchCode+"'>"+
		...
		loxia.asyncXhrPost(window.parent.$j("body").attr("contextpath")+"/json/executeinventory.json",postData,
		{
			success:function (data) {
				if(data&&data.message){
					...
					window.location=$j("body").attr("contextpath")+"/warehouse/initToVMIFlittingEnter.do";;
				}	
			}, 
	       error:function(){
				 jumbo.showMsg(i18("MSG_FAILURE"));	
