...
$j(document).ready(function (){
	...
	$j("#detail_tbl_one").jqGrid({
		datatype: "json",
		colNames:["商品ID","SKU编码","条形码","商品名称","数量","商品状态","操作"],
		colModel:[
...
function commitRequest(){
	...
	for(var i=0;i<ids.length;i++){
		...
		var quantity = $j("#q_" + skuId).val();
		var status = $j("#s_" + skuId).val();
		if(quantity == "" || quantity == "0"){
			jumbo.showMsg("扫描录入商品列表第【"+(i+1)+"】行商品数量不能为0或空！");
			return;
		}
		skuString1 += skuId+"," + quantity + ";" + status + "/";
	}
...
function addSku(obj, isAdd){
	...
	if (!isAdd){
		$j("#q_"+obj.skuId+"").attr("value",obj.quantity);
		$j("#s_"+obj.skuId+"").attr("value",obj.invStatusId);
	}
}
