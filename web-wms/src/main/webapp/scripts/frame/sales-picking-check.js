...
function showDetail(obj,plid){
	var plCode = $j("#quickPl").val();
	$j("#quickPl").val("");
	var row = $j(obj).parent().parent().attr("id");
	if(plid){
		row = plid;
		var pl=$j("#tbl-dispatch-list").jqGrid("getRowData",row);
		...
		var pl = loxia.syncXhrPost($j("body").attr("contextpath") + "/findPackinglistByCode.json",{"plCmd.code":plCode});
		if(pl && pl["pl"]){
		row = pl['pl']['id'];
	...
	$j("#quickPl").val("");
	$j("#tbl-sta-list").jqGrid('setGridParam',{url:loxia.getTimeUrl($j("body").attr("contextpath") + "/findocpstalbypl.json"),
...
$j(document).ready(function (){
	...
	$j("#tbl-sta-list").jqGrid({
		...
		colModel: [{name: "id", index: "id", hidden: true,sortable:false},
					...
					{name:"intStatus", index:"intStatus" ,width:40,resizable:true,formatter:'select',editoptions:stastatus},
					...
					{name:"barCode",width:100,resizable:true,sortable:false},
	...
	$j("#quickPl").keydown(function(evt){
		if(evt.keyCode === 13){
			...
			var isFinish = false;
			var plid = null;
			var data = $j("#tbl-dispatch-list").jqGrid('getRowData');
			$j.each(data,function(i, pl){
				if(!isFinish && pl["code"] == plcode){
					isFinish = true;
					plid = pl["id"];
				}
			});
