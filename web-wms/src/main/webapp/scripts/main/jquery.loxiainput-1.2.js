(function($) {
	var loxiaInput = $.extend({}, loxia.loxiaWidget, {
		...
		_initInput : function(){
			...
			var _this = this;
			var defaultAcpOptions = {
					select: function(event, ui){
						if(ui.item)
							_this.val(ui.item.value ? ui.item.value : ui.item);
					}
				};
			if(this.option("autocomplete")){
				var acp = this.option("autocomplete");
				if(loxia.isString(acp)){
					//url
					var acpoptions = $.extend({
						source: acp,
						minLength: 2
					},defaultAcpOptions);
					//console.log(acpoptions);
					this.element.autocomplete(acpoptions);
				}else{
					var acpoptions = $.isArray(acp) ? {source: acp} : acp;
					if(acpoptions.select == undefined){
						$.extend(acpoptions,defaultAcpOptions);
					}
					//console.log(acpoptions);
					this.element.autocomplete(acpoptions);
				}
		...
		_create : function(){
			...
			if(this.element.is("input") || this.element.is("textarea")){
				...
				var acp = this.element.attr("aclist");
				if(acp){					
					this.option("autocomplete",loxia.global[acp] ? loxia.global[acp] : acp);					
				}
				this._initInput();
