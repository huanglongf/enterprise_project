	...
	var loxiaEditTable = {
		...
		_initActionBar: function(){
			...
			if(opstr.indexOf("add") >=0)
				bar += '<button type="button" icons="ui-icon-circle-plus" loxiaType="button" action="add" title="' + loxia.getLocaleMsg("TABLE_BARBTN_ADD") + '">' + loxia.getLocaleMsg("TABLE_BARBTN_ADD") + '</button>';
			if(opstr.indexOf("delete") >=0)
				bar += '<button type="button" icons="ui-icon-circle-minus" loxiaType="button" action="delete" title="' + loxia.getLocaleMsg("TABLE_BARBTN_DELETE") + '">' + loxia.getLocaleMsg("TABLE_BARBTN_DELETE") + '</button>';
			
			$('<div class="tbl-action-bar ui-state-default">' + bar + '</div>').prependTo(this.element);
