...
<script type="text/javascript" src="<s:url value='/scripts/frame/warehouse/pickinglist-secondary.js"' includeParams='none' encode='false'/>"></script>
	...
	<form action="/generationPendingList.json" method="post" id="query-form">
		...
		<table id="filterTable" width="95%">
				<tr>
					...
					<td width="20%"><input loxiaType="date" name="fromDate" id="fromDate" showtime="true"/></td>
					<td class="label" width="13%"><s:text name="label.warehouse.pl.to"></s:text> </td>
					<td width="20%"><input loxiaType="date" name="toDate" id="toDate" showtime="true"/></td>
				...
				<tr>
					...
					<td>
						<select name="sta.specialType" id="selIsSpPg" loxiaType="select">
				...
				<tr>
					...
					<td class="label"  style="color: red;">是否QS</td>
					<td>
						<select  name="sta.isSpecialPackaging" id="selIsQs" loxiaType="select">
				...
				<tr>
					...
					<td class="label" hidden>是否包含单件</td>
					<td  hidden><input type="checkbox" id="issingle"/></td>
				...
				<tr>
					<td class="label">SKU编码（1）</td>
					<td>
						<input loxiaType="input" trim="true" name="sta.code1" id="code1"/>
					</td>
					<td class="label">SKU编码（2）</td>
					<td>
						<input loxiaType="input" trim="true" name="sta.code2" id="code2"/>
					</td>
					<td class="label">SKU编码（3）</td>
					<td>
						<input loxiaType="input" trim="true" name="sta.code3" id="code3"/>
					</td>
				</tr>
				<tr>
					<td class="label">SKU编码（4）</td>
					<td>
						<input loxiaType="input" trim="true" name="sta.code4" id="code4"/>
					</td>
					<td class="label">按钮高亮显示</td>
					<td>
						<select name="sta.pickSubType" id="pickSubType" loxiaType="select">
							<option value="">--请选择--</option>
							<option value="1">拣货模式1</option>
							<option value="2">拣货模式2</option>
						</select>
					</td>
					<td class="label" style="color:red">包装类型</td>
					<td>
						<select name="sta.packingType" id="packingType" loxiaType="select">
							<option value="">--请选择--</option>
							<option value="GIFT_BOX">礼盒包装</option>
						</select>
					</td>	
				</tr>
				<tr>
					...
					<td class="label"  style="color: red;" width="13%">订单类型</td>
					<td width="20%">
						<div>
							<select name="sta.orderType2" id="orderType2" loxiaType="select">
							<option value="" selected="selected">普通</option>
							<option value="1">MSR定制订单</option>
							</select>
						</div>
					</td>
				...
				<tr>
					...
					<td colspan="5">
							...
							忽略已选拣货区域查询条件
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<s:if test="CountAreas"><input type="checkbox" value="<s:property value='CountAreas'/>" checked="checked" id="CountAreas" name="CountAreas"/></s:if>
									<s:else>
										<input type="checkbox" value="<s:property value='CountAreas'/>" id="CountAreas" name="CountAreas"/>
							</s:else>
							混合拣货区域
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<s:if test="CountAreasCp"><input type="checkbox" value="<s:property value='CountAreasCp'/>" checked="checked" id="CountAreasCp" name="CountAreasCp"/></s:if>
									<s:else>
										<input type="checkbox" value="<s:property value='CountAreasCp'/>" id="CountAreasCp" name="CountAreasCp"/>
							</s:else>
							混合拣货区域（包含所有排列组合）
					</td>
				</tr>
				...
				<tr>
					...
					<td width="20%"><input loxiaType="date" name="orderCreateTime" id="orderCreateTime" showtime="true"/></td>
					<td class="label" width="13%"><s:text name="label.warehouse.pl.to"></s:text> </td>
					<td width="20%"><input loxiaType="date" name="toOrderCreateTime" id="toOrderCreateTime" showtime="true"/></td>
					<td class="label" style="color: blue;">O2O门店</td>
					<td>
						<select name="sta.toLocation" id="selToLocation" loxiaType="select">
				...
				<tr>
				   ...
				   <td><input type="checkbox"   id="otoAll" name="otoAll"/>所有O2O门店</td>
				</tr>
				<tr>
				   <td class="label" width="13%">规则名称</td>
				   <td >
...
<div id="staLineInfo" class="hidden">
	...
	<table>
		<tr>
			<td class="label"  class="label" style="color: red;">ps:平台订单相关时间:非COD订单为付款时间、COD订单为创建时间</td>
		</tr>
	</table>
</body>
</html>