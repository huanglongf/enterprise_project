...
function initRuleCodeList(){
	...
	for(var i in staLineList){
		$j("#"+staLineList[i].ruleCode).css("background-color","");
	}
...
function checkedOrRecord(code,staLineId){
	var rs=loxia.syncXhr($j("body").attr("contextpath") + "/twicePickingByBarcode.json?pickinglistId="+$j("#plid").val()+"&code="+code+"&stalineId="+staLineId+"&isNeedCheck=false");
	if(rs.result=="success"){
		...
		if(data.isLineOver){
			...
			for(var j=0;j<ids.length;j++){
				if(item.ruleCode==$j("#tbl-detail-list").getCell(ids[j],"ruleCode")){
					b=true;
					break;
				}
			}
			...
			if(ids2==null||ids2.length==0){
				loxia.syncXhr($j("body").attr("contextpath") + "/resetBoxAndCollection.json?pickinglistId="+$j("#plid").val());
				$j("#showinfoOver").html("批次[ " + $j("#plCode").html() + " ]拣货完成");
...
function printOrder(staId){
	...
	var url = $j("body").attr("contextpath") + "/printSingleOrderDetail1.json?id=" +staId;
