...
function showDetail(obj){
	...
	$j("#tbl-detail-list").jqGrid('setGridParam',{url:loxia.getTimeUrl($j("body").attr("contextpath") + "/findStaLineByPickingIdopc.json"),
			postData:{"pickinglistId":row,"ou.id":wid}}).trigger("reloadGrid",[{page:1}]);
}
$j(document).ready(function (){
	...
	$j("#tbl-dispatch-list").jqGrid({
		url:baseUrl + "/pickingListForPickOutopc.json",
		datatype: "json",
		colNames: ["ID","配货批单号","状态","创建人","创建时间","计划单据数","所属仓库","仓库id"],
		colModel: [{name: "id", index: "p.id", hidden: true},
					...
					{name:"planBillCount",index:"p.plan_bill_count",width:80,resizable:true},
					{name:"wname",index:"wname",width:120,resizable:true},
					{name:"wid",index:"wid",width:80,resizable:true,hidden:true}
				  ],
		caption: "配货批列表",
	   	sortname: 'id',
	   	multiselect: false,
	...
	jumbo.bindTableExportBtn("tbl-dispatch-list",{"intStatus":"pickingListStatus"},baseUrl + "/pickingListForPickOutopc.json");
	...
	jumbo.bindTableExportBtn("tbl-detail-list",{"intStatus":"whSTAStatus"},baseUrl + "/findStaLineByPickingIdopc.json");
...
//		}
	...
	$j("#pickinglistCodeQuery").keydown(function(event){
		if(event.keyCode == 13){
			...
			if(code != null && code != ""){
				...
				result = loxia.syncXhr($j("body").attr("contextpath") + "/checkPinkingListByCodeopc.json",postData);
				if(result.result=='success'){
					...
					$j("#plCreateUser").html(data.crtUserName);
					$j("#plBillCount").html(data.planBillCount);
					$j("#tbl-detail-list").jqGrid('setGridParam',{url:loxia.getTimeUrl($j("body").attr("contextpath") + "/findStaLineByPickingIdFast.json"),
	...
	$j("#search").click(function(){
		...
		$j("#tbl-dispatch-list").jqGrid('setGridParam',{url:loxia.getTimeUrl($j("body").attr("contextpath") + "/pickingListForPickOutbywid.json"),
