...
var isSkipWeight="0";//是否跳过称重
var isNeedWrapStuff;//是否扫耗材
var isSingle=true;
var isLast=true;
var rfidNum=0;
var rfidList={};
...
function init(){
	...
	if(isCheck){
		//判断是否复核,复核进入1_1
		step1_1();
	}else if(isWeight&&isSkipWeight=="0"){
		//判断是否称重,称重进入2_1
		step2_1();
	}
	//判断是否交接
...
function initParam(){
	...
	if(rswh && rswh.warehouse){
		if (rswh.warehouse.isNeedWrapStuff&&isSkipWeight=="0"){
			isMate = true;
		}
...
function initStep1_2(orderCode){
	...
	if(isSkipWeight=="1"&&orderList.orders[0].pickingType==1&&tableData[0].qty==1&&isNeedWrapStuff==true){
		$j("#LabelMaterialSingle").removeClass("hidden");
		$j("#txtMaterialSingle").removeClass("hidden");
		$j("#btnMaterialSingle").removeClass("hidden");
		isSingle=true;
	}else{
		$j("#LabelMaterialSingle").addClass("hidden");
		$j("#txtMaterialSingle").addClass("hidden");
		$j("#btnMaterialSingle").addClass("hidden");
		isSingle=false;
	}
}
...
function checkScanSku(barcode,sn){//run
	...
	if(isSkipWeight=="1"&&isSingle==false&&toCheckData.orders.length==1&&toCheckData.orders[0].skuQty==1&&isLast==true&&isNeedWrapStuff==true){
		$j("#dialogMaterial").dialog("open");
		isLast=false;
		return false;
	}
...
function checkOrder(nowOrder){
	...
	var txtMaterialSingle=$j("#txtMaterialSingle").val();
	postData["isSkipWeight"]=isSkipWeight;
	if(txtMaterialSingle!=""||txtMaterialSingle!=null){
		postData["staffCode2"]=txtMaterialSingle;
	}
...
function outbound(weight){
	...
	//非核对中称重做出库操作
		...
		if(isSkipWeight=="1"){
			...
			postData["trackingNo"]=$j("#labTransNo").html().trim();
		}else{
...
$j(document).ready(function() {
	...
	$j("#dialogMaterial").dialog("close");
	var warehouse=loxia.syncXhrPost(BASE_URL+ "/findWareHouseById.json");
	isSkipWeight=warehouse.warehouse.isSkipWeight;
	isNeedWrapStuff=warehouse.warehouse.isNeedWrapStuff;
	if(isSkipWeight==true){
		$j("#isW").attr("disabled","disabled");
		isSkipWeight="1";
	}else{
		isSkipWeight="0";
	}
	
	$j("#LabelDeviceNumber").addClass("hidden");
	$j("#txtDeviceNumber").addClass("hidden");
	$j("#btnDeviceNumber").addClass("hidden");
	$j("#LabelDeviceOk").addClass("hidden");
	$j("#txtDeviceOk").addClass("hidden");
	$j("#btnDeviceOk").addClass("hidden");
	
	$j("#LabelRfidCode").addClass("hidden");
	$j("#btnRfidCode").addClass("hidden");
	/******************************************************************************/
	/**对话框                                                                     */
	/******************************************************************************/
	//特殊处理
	...
	$j("#dialogMaterial").dialog({title: "扫描耗材", modal:true,closeOnEscape: false,autoOpen: false, width: 450,		open : function(event, ui) {	$j(".ui-dialog-titlebar-close").hide();	},
	});
	//错误信息对话框
	$j("#dialogMsg").dialog({
		title : "错误信息",
	...
	$j("#btnErrorOk").click(function(){
		...
		if(msgType == 1){
			...
			msgType = 0;
			if(isSkipWeight=="1"&&isNeedWrapStuff==true){
				$j("#dialogMaterial").dialog("open");
				$j("#material").val("");
				return false;
			}
	...
	$j("#btnScanSku").click(function(){
	    ...
	    var postDataRfid={};
	    postDataRfid["barcode"]=barcode;
	    var sku=loxia.syncXhrPost(BASE_URL+ "/findSkuRfidByBarCode.json",postDataRfid);
	    var ifRifd=sku.sku.rfid;
	    rfidNum=sku.sku.rfidNumber;
	    if(ifRifd==true){
	    	$j("#LabelDeviceNumber").removeClass("hidden");
	    	$j("#txtDeviceNumber").removeClass("hidden");
	    	$j("#btnDeviceNumber").removeClass("hidden");
	    	$j("#LabelDeviceOk").removeClass("hidden");
	    	$j("#txtDeviceOk").removeClass("hidden");
	    	$j("#btnDeviceOk").removeClass("hidden");
	    	$j("#txtDeviceNumber").focus();
	    	return false;
	    }else{
	    	$j("#LabelDeviceNumber").addClass("hidden");
	    	$j("#txtDeviceNumber").addClass("hidden");
	...
	//【订单号】点击
	$j("#btnScanSlipCode").click(function(evt){
		var slipCode = $j("#txtScanSlipCode").val().trim();
		if(slipCode==""){
			showErrorMsg("请扫描订单号！");
			return false;
	...
	//【耗材单】点击
	$j("#btnMaterialSingle").click(function(){
		var mate = $j("#txtMaterialSingle").val().trim();
		if(mate==""){
			showErrorMsg("扫描耗材为空！");
			return false;
		}
	...
	//【耗材单】扫描
	$j("#txtMaterialSingle").keypress(function(evt){
		if (evt.keyCode === 13) {
			$j("#btnMaterialSingle").trigger("click");
		}
	});
	...
	//【耗材多】点击 wangqian
	$j("#btnMaterial").click(function(){
		var mate = $j("#material").val().trim();
		if(mate==""){
			showErrorMsg("扫描耗材为空！");
			return false;
		}
		...
		var postData = {};
		postData["transNo"]=$j("#labWeightTransNo").html();
		postData["weight"]=0;
		postData["staffCode"]=mate;
		postData["isSkipWeight"]=isSkipWeight;
		if(isLast==true){
			var rs = loxia.syncXhrPost(loxia.getTimeUrl(BASE_URL+"/recordweightfortrackingno.json"),postData);
		}
	    isSpPackige=true;
	    $j("#dialogMaterial").dialog("close");
	    $j("#txtMaterialSingle").val($j("#material").val().trim());
	    $j("#material").val("");
		if(isLast==false){
			var barcode = $j("#txtScanSku").val().trim();
		    var sn = $j("#txtScanSn").val().trim();
	...
	//【耗材多】扫描
	$j("#material").keypress(function(evt){
		if (evt.keyCode === 13) {
			$j("#btnMaterial").trigger("click");
		}
	});
	...
	//【设备号】点击
	$j("#btnDeviceNumber").click(function(){
		$j("#txtDeviceOk").focus();
	});
	//【设备号】扫描
	$j("#txtDeviceNumber").keypress(function(evt){
		if (evt.keyCode === 13) {
			$j("#btnDeviceNumber").trigger("click");
		}
	});
	
	
	//【OK】点击
	$j("#btnDeviceOk").click(function(){
		
		var URL="http://10.8.35.228:8088";
		var hostCode=$j("#txtDeviceNumber").val().trim();
		var rfid=loxia.syncXhr(URL + "/getRfid",{"hostCode":hostCode});
		if(rfid.rfid.length==rfidNum){
			$j("#LabelRfidCode").html("RFID号:"+rfid.rfid[0]+"|"+rfid.rfid[1]);
			$j("#LabelRfidCode").removeClass("hidden");
			$j("#btnRfidCode").removeClass("hidden");
			alert(rfid.rfid);
			rfidList=rfid.rfid;
		}else{
			return false;
	...
	//【OK】扫描
	$j("#txtDeviceOk").keypress(function(evt){
		if (evt.keyCode === 13) {
			$j("#btnDeviceOk").trigger("click");
		}
	});
	
	$j("#btnRfidCode").click(function(){
		$j("#LabelDeviceNumber").addClass("hidden");
		$j("#txtDeviceNumber").addClass("hidden");
		$j("#btnDeviceNumber").addClass("hidden");
		$j("#LabelDeviceOk").addClass("hidden");
		...
		var postDataRfid={};
	    postDataRfid["rfid"]=rfidList;
	    postDataRfid["skuBarCode"]=$j("#txtScanSku").val().trim();
		var rfidResult = loxia.syncXhrPost(BASE_URL + "/findRfidResult.json",postDataRfid);
		return false;
