...
<script type="text/javascript" src="<s:url value='/scripts/frame/sta_inboundhand.js' includeParams='none' encode='false'><s:param name="version"><s:property value="#VERSION" /></s:param></s:url>"></script>
<style>
.showborder {
	border: thin;
}
...
<!-- 这里是页面内容区 -->
	...
	<div id="buttonlist">
		<table>
			<tr><td><button loxiaType="button" class="confirm" id="confirm">确认本次收货</button></td><td><button loxiaType="button" id="back">返回</button></td></tr>
		</table>
	</div>
	<br/>
	<div id="buttonlist">
		<table>
			<tr>
				<td><button loxiaType="button" class="confirm" id="printContainerCode">打印货箱标签</button></td>
				<td><button loxiaType="button" class="confirm" id="printSlipCode">打印作业单号</button></td>
			</tr>
			<tr>
				<td><input type="text" loxiaType="input" trim="true" id="printInput" ></input></td>
				<td><font color="red">输入'OK'将会自动打印货箱标签，输入'NO'将会自动打印作业单号</font></td>
			</tr>
		</table>
	</div>
