...
function reloadSta(plId,wid){
	var 	status = "2";
	$j("#tbl-showDetail").jqGrid('setGridParam',{url:loxia.getTimeUrl($j("body").attr("contextpath") + "/getPickCheckListopc.json"),
			postData:{"cmd.id":plId,"status":status,"ou.id":wid}}).trigger("reloadGrid",[{page:1}]);
	status = "3,4,10";
	$j("#tbl-showReady").jqGrid('setGridParam',{url:loxia.getTimeUrl($j("body").attr("contextpath") + "/getPickCheckListopc.json"),
			postData:{"cmd.id":plId,"status":status,"ou.id":wid}}).trigger("reloadGrid",[{page:1}]);
	//取消列表
	status = "15,17";
	$j("#tbl-showCancel").jqGrid('setGridParam',{url:loxia.getTimeUrl($j("body").attr("contextpath") + "/getPickCheckListopc.json"),
		postData:{"cmd.id":plId,"status":status,"ou.id":wid}}).trigger("reloadGrid",[{page:1}]);
}
...
function doCheck(obj){
	...
	$j("#staId").val(sta['id']);
	var date = loxia.syncXhrPost(window.$j("body").attr("contextpath")+"/json/getWarehouseInfo.json",{"ou.id":$j("#verifyWid").val()});
	if(date.isMqInvoice){
...
$j(document).ready(function (){
	...
	if(!plstatus.exception){
		$j("#tbl-dispatch-list").jqGrid({
			url:$j("body").attr("contextpath") + "/searchCheckListBycenid.json",
			postData:{"cmd.sortingModeInt":2},
			...
			colNames: ["ID",i18("CODE"),i18("INTSTATUS"),i18("PLANBILLCOUNT"),i18("CHECKEDBILLCOUNT"),i18("SHIPSTACOUNT"),i18("PLANSKUQTY"),i18("CHECKEDSKUQTY"),i18("SHIPSKUQTY"),
				i18("CREATE_TIME"),i18("CHECKEDTIME"),i18("EXECUTEDTIME"),i18("PICKINGTIME"),i18("WH_NAME"),i18("仓库id")],
			colModel: [{name: "id", index: "pl.id", hidden: true},
		               ...
		               {name:"wid",index:"",width:120,resizable:true,hidden:true}],
	...
	$j("#reloadSta").click(function(){
		...
		reloadSta($j("#showDetail").attr("plId"),$j("#showDetail").attr("wid"));
	});
	...
	$j(".refSlipCode").keydown(function(evt){
		if(evt.keyCode === 13){
			...
			loxia.lockPage();
			loxia.asyncXhrPost($j("body").attr("contextpath") + "/staSlipCodeCheck.json",
