...
//	$j("#dialog_error_ok1").dialog({title: "错误信息", modal:true, autoOpen: false, width: 400});
	...
	if(!plstatus.exception){
		$j("#tbl-dispatch-list").jqGrid({
			url:$j("body").attr("contextpath") + "/searchPickingListByBig.json",
	...
	$j("#search").click(function(){
		$j("#tbl-dispatch-list").jqGrid('setGridParam',{url:loxia.getTimeUrl($j("body").attr("contextpath") + "/json/searchPickingListByBig.json"),
	...
	$j(".refSlipCode").keydown(function(evt){
		if(evt.keyCode === 13){
			...
			loxia.lockPage();
			loxia.asyncXhrPost($j("body").attr("contextpath") + "/json/staSlipCodeCheckBig.json",
				{"sta.refSlipCode":refSlipCode,
				 "sta.pickingCode":$j("#iptPlCode").val()},
				{success:function (data) {
					loxia.unlockPage();
					if(data&&data.id && (data.intCheckMode == 1 || data.intCheckMode == 5 ||  data.intCheckMode == 10 ||  data.intCheckMode == 2)){
					try{
						closeGrabberJs();				
					}catch (e) {
...
//														postData:{"sta.id":$j("#staId").val()}}).trigger("reloadGrid",[{page:1}]);
	...
	$j("#express_order").keydown(function(evt){
		...
		if(evt.keyCode === 13){
			...
			var rs = loxia.syncXhrPost($j("body").attr("contextpath")+ "/operationUnitQuery.json");
			if(rs && rs.result=="success"){
				$j("#goodsWeigth").attr("isManualWeighing",rs.warehouse["isManualWeighing"]);
				$j("#wrapStuff").attr("isNeedWrapStuff",rs.warehouse["isNeedWrapStuff"]);
			}
...
//				$j(this).select();
			...
			if(isOver){
				var wrapStuff=$j("#wrapStuff").attr("isNeedWrapStuff");
				if(wrapStuff=="true"){
					$j("#wrapStuff").focus();
				}else{
				var goodsWeigth=$j("#goodsWeigth").attr("isManualWeighing");
				if(goodsWeigth=="true"){
					$j("#goodsWeigth").focus();
				}else{
...
//				var weigh = loxia.syncXhrPost(window.$j("body").attr("contextpath")+"/checkIsWeigh.json");
//				if(weigh.result=='success'){
//					openBigLuxuryWeighDialog();
//				}else if(weigh.result=='exception'){
//					errorTipOK("系统异常，无法识别该仓库是否使用大件复核称重功能！");
...
//大件、奢侈品仓：复核后直接称重打印面单
function openBigLuxuryWeighDialog(){
	$j("#if_weigh").attr("src","bigLuxuryWeigh.do"); //设置IFRAME的SRC;
    $j("#dialog-weigh").dialog({
        bgiframe: true,
