...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript" src="<s:url value='/frame/sortingCheck/sales-dispatch-list-o2oqs-query.js"' includeParams='none' encode='false'><s:param name="version"><s:property value="#VERSION" /></s:param></s:url>"></script>
	</head>
	<body style="background-color: #f2f2f2;" contextpath="<%=request.getContextPath() %>">
		<s:text id="pselect" name="label.please.select"/>
		<input type="hidden" id="backTarget" />
		<input type="hidden" id="staId"/>
		<input type="hidden" id="lpcode"/>
		<input type="hidden" id="plpId"/>
		<input type="hidden" id="trackingNo"/>
		<!-- 配货清单列表 -->
		<div id="showList">
			<div>
			    <form id="plForm" name="plForm">
			    	<input type="hidden" id="sortingModeInt" name="cmd.sortingModeInt" value="3"/>
					<table width="70%">
						<tr>
			...
			<div style="margin-bottom: 10px;">
				<span class="label"><s:text name="label.warehouse.pl.verify"/></span>输入批次号后按回车进入核对界面
				<br/>
				...
				<input id="refSlipCode" class="refSlipCode" loxiaType="input" style="width: 150px" trim="true"/>&nbsp;  --%>
					...
					<input id="iptPlCode" class="iptPlCode" loxiaType="input" style="width: 150px" trim="true"/>
			</div>
			<div>
		...
		<table>
			...
			<td>
				<table width="700px">
				<tr>
					<td class="label" width="20%" style="font-size: 24px"><s:text name="label.warehouse.pl.batchno"/><input type="hidden" id="verifyPlId"/>
					<input type="hidden" id="isPostpositionPackingPage"/><input type="hidden" id="isPostpositionExpressBill"/>
					</td>
					<td width="25%" id="verifyCode" style="font-size: 24px"></td>
					<td class="label" width="20%" style="font-size: 24px"><s:text name="label.warehouse.pl.status"/></td>
					<td width="25%" id="verifyStatus" style="font-size: 24px"></td>
					<td class="label">&nbsp;</td>
					<td>&nbsp;</td>
				</tr>
				<tr>
			...
			<br /><br />
			<div id="doCheckDetail" class="hidden">
		<%-- 	<div style="margin-top: 20px;margin-bottom: 15px;">
				<table>
						<tr>
							  <td>
								...
								<td>
								<select id="selLpcode" name="lpcode" loxiaType="select"></select>
								</td>
								<td>
			...
			<table>
				<tr>
					...
					<td>
						<table id="tbl-trank-button" style="margin-top:20px;" class="hidden">
							<tr height = "38px">
								<td  valign="top">
			...
			<div class="buttonlist">
		 	...
		 	<table id="tbl-showPackaged"></table>
			<br /><br />
			<table id="tbl-showReady" class="hidden"></table>
			<br /><br />
		...
		<div>
				<div id="sn_import">
					<span class="label" style="float:left;">　SN号导入</span>
			...
			<div style="margin-bottom: 15px;margin-top: 25px;">
				...
				<span><input id="barCode_11" loxiaType="input" style="width: 200px;height: 25px" trim="true"/></span>
				...
				<tr>
					<td>
						<div style="margin-top: 20px;margin-bottom: 15px;">
							...
							<input id="express_order_11" style="width: 200px;height: 22px" loxiaType="input" />
							<button id="printDeliveryInfo_11" class="confirm ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" role="button" aria-disabled="false">
			                <span class="ui-button-text">打印物流面单</span>
			                </button>
						</div>
						<table id="tbl-express-order_11" width="500px"  style="text-align: center;" border="1px" cellpadding="0px" cellspacing="0px">
							<tr>
								<td class="label" height="30" style="text-align: center;"><s:text name="label.warehouse.pl.sta.delivery.code"/></td>
								<td class="label" height="30" style="text-align: center;"><s:text name="label.warehouse.pl.sta.operation"/></td>
							</tr>
					...
					<td>
						<table id="tbl-trank-button_11" style="margin-top:20px;">
							<tr height = "38px">
								<td  valign="top">
									<button loxiaType="button" class="confirm" id="addTrank_11">新增运单</button>
								</td>
							</tr>
							<tr>
								<td>
									<table id="tbl-trank-list_11" width="500px"  style="text-align: center;" border="1px" cellpadding="0px" cellspacing="0px">
										<tr>
											<td class="label" height="30" style="text-align: center;">拆包运单号</td>
											<td class="label" height="30" style="text-align: center;"><s:text name="label.warehouse.pl.sta.operation"/></td>
										</tr>
			...
			<div class="buttonlist">
				<button loxiaType="button" class="confirm" id="doCheck_11"><s:text name="label.warehouse.pl.executeverify"/></button>
				<button loxiaType="button" id="Back_11"><s:text name="button.toback"/></button>
			           确认条码：<input id="barCode1_11"  loxiaType="input" style="width: 100px" trim="true" maxlength="6"/>
			</div>
		</div>
