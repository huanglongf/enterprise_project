...
<script type="text/javascript" src="../scripts/frame/picking-list-query.js"></script>
</head>
...
<div id="div1">
	<form id="quert-form" method="post">
		<table width="90%">
			...
			<tr>
				<td class="label">是否O2O批次</td>
				<td>
					<select id="otoPick" name="comd.otoPick" loxiaType="select">
						<option value=""><s:text name="label.wahrhouse.sta.select"></s:text> </option>
...
<%-- 			<td class="label"><s:text name="label.wahrhouse.pl.last.send.time"></s:text> </td>
	...
	<br/>
	<div class="buttonlist">
		<c:if test="${param.showExport}">
			<button loxiaType="button" class="confirm" id="exportDeliveryInfo"><s:text name="button.trans.export"></s:text></button>
			<button loxiaType="button" id="exportDeliveryInfoZip"><s:text name="button.trans.export"></s:text>压缩文件</button><br><br>
		</c:if>
		<button loxiaType="button" class="confirm" id="printPickingList"><s:text name="button.pg.print"></s:text> </button>
