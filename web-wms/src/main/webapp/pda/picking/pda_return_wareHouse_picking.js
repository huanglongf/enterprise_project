		...
		$(document).ready(function(){
			...
			$("#boxCodeOk").click(function(){
				...
				boxCode = $("#boxCode").val() ;
				var postData = {
						"code":$("#pickingBarCode").val(),
						"cCode":boxCode						
				}; 
				//判断此货箱是否可用
				var flag=request($("body").attr("contextpath") + "/pda/returnWhCheckBox.do",postData);
				if(flag["flag"]=="success"){
					if(flag["msg"]!=null&&flag["msg"]!=""){
						//alert(flag["msg"]);
						//playMusic($("body").attr("contextpath")+"/pda/images/1.wav");
...
//			//确认商品短拣
//			$("#skuShortOK1").click(function(){
//				toSkuShort1();
//				//获取作业单code
//				var postData = {
//						"code":pickingBarCode
//				}; 
//				var flag=request($("body").attr("contextpath") + "/pda/getStaCodeBypbBatchCode.do",postData);
//				$("#pbShortCount").text(wp.qty);
//				$("#pbCode1").text(pickingBarCode);
//				if(flag!=null&&flag!=''){
//					$("#staCode").text(flag.staCode);
//				}
//			});
			//确认商品短拣
			...
			//显示带链接的短拣信息页面
			$("#showShortDiv").click(function(){
				checkPickingBatchOverByShort();
		...
		function setSkuInfo(){
			...
			var skuInvStatus = wp.skuStatus;
			if(null == skuInvStatus || ""==skuInvStatus){
				$("#skuInvStatusTr").hide();
			}else{
				$("#skuInvStatus").text(skuInvStatus);
			}
...
//					toPbShort();
		...
		//校验二级批次拣货完成
		function checkPickingBatchOverByShort(){
			//查询是否有商品短拣
			pickingCode = temporaryWp.code;
			var postData = {
					"code":pickingCode						
		...
		function pickingBatchOver(){
			...
			var flag=request($("body").attr("contextpath") + "/pda/returnWhousePickingBatchOver.do",postData);
			if(flag["flag"]=="success"){
					isAutoSingle=true;
				if(flag["msg"]!=null&&flag["msg"]!=""){
					//alert(flag["msg"]);
