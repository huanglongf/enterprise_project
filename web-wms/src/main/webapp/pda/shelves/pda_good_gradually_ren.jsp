				...
				<div class="am-tabs " data-am-tabs="" id="good1">
					<div class="am-tabs-bd" style="-webkit-user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">
						<div class="am-tab-panel am-fade  am-in" >
							<div class="am-g">
						    <div  style="font-size:15px;text-align: left;">&nbsp;&nbsp;扫描容器号(良品逐件(人))</div>
							</div>
							<div class="am-g">
		...
		$(document).ready(function(){
			...
			$("#skuCode").keypress(function(evt){
				if (evt.keyCode === 13) {
					var key = $("#skuCode").val();
					if(key!=""){
						var skuCode3=$("#skuCode3").text();
						var skuCode=$("#skuCode").val();
						var skuCodeNum=1;
						if(skuCode!=null&&(skuCode3==null||skuCode3=="")){
						if(skuCodeNum=="0"){
							$("#msg").text("请扫描商品编码");
						}else{
							//调用该商品是否是效期商品
						...
						var flag=request($("body").attr("contextpath") + "/pda/checkSkuShouPro.do",postData);
						if(flag["flag"]=="error"){
							$("#skuCodeNum").text("0");
							$("#skuCode3").text("");
							$("#scanShow").hide();
						...
						if(skuCode3==""){
							...
							$("#skuCodeNum").text("1");
							$("#skuCode").val("");
						}else if(key!=skuCode3){
							//$("#msg").text("请扫"+skuCode3);
							$("#skuCode").val("");
							//$("#msg").text("");
							alert("该条码:"+skuCode+"不在此箱内");
						}else{
							$("#skuCodeNum").text(parseInt($("#skuCodeNum").text())+1);
							$("#skuCode").val("");
							$("#msg").text("");
						}
					}else {
			...
			$("#goodOk2").click(function(){//扫描商品条码 确认
				var skuCode=$("#skuCode3").text();
				var skuCodeNum=$("#skuCodeNum").text();
				if(skuCodeNum=="0"){
					$("#msg").text("请扫描商品编码");
				}else{
					//调用该商品是否是效期商品
				...
				if(flag["flag"]=="success"){
					$("#isQx").text(flag["isXq"]);
					   	$("#skuCode2").text($("#skuCode3").text());
						$("#skuCodeNum2").text($("#skuCodeNum").text());
						$("#eDate").val("");
						$("#sDate").val("");
						toBox(3);
						if(flag["isXq"]=="1"){
