				...
				<div class="am-tabs " data-am-tabs="" id="good4">
					<div class="am-tabs-bd" style="-webkit-user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">
						<div class="am-tab-panel am-fade  am-in" >
							...
							<div class="am-g">
								<div >扫描sku&nbsp;&nbsp;&nbsp;计数：<font id="qty1"></font>-<font id="qty2"></font></div>
							</div>
							<div class="am-g">
							<div class="input-element-frame">
		...
		$(document).ready(function(){
			...
			$("#skuBarCode").keypress(function(evt){//enter skuBarCode
				...
				if(skuBarCode==""){
					...
					var flag=request($("body").attr("contextpath") + "/pda/scanSku.do",postData);//验证商品来更新 插入作业单操作明细
					if(flag["flag"]=="success"){
						if(flag["line"]==null){
							alert("line is null");
						}else{
							...
							$("#skuBarCode").val("");
							$("#msg").text("");
							if(flag["line"]["remainQty"]=='0'){//自动判断跳转
								if(flag["isOver"]=='1'){
			...
			$("#goodBack4").click(function(){
				...
				var flag=request($("body").attr("contextpath") + "/pda/cancelSku.do",postData);//扫描商品取消回滚
				if(flag["flag"]=="success"){
					toBox(3);
					$("#locationCode").val("");
				}else if(flag["flag"]=="error"){
