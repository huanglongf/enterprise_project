...
public class VmiDefault extends VmiBaseBrand {
        ...
        if (sta != null && stv != null) {
            ...
            if (bi.getAsnTypeString().equals(String.valueOf(AsnOrderType.TYPEONE.getValue()))) {
                ...
                if (bi.getRsnTypeString().equals(String.valueOf(RsnType.TYPETWO.getValue()))) {
                    ...
                    List<StockTransTxLogCommand> logList = null;
                    if (bi.getIsPda() != null && bi.getIsPda()) {// ispda 为true
                        logList = stvLogDao.findStaLogByStvId2(sta.getId(), new BeanPropertyRowMapper<StockTransTxLogCommand>(StockTransTxLogCommand.class));
                    } else {
                        logList = stvLogDao.findStaLogByStvId(stv.getId(), new BeanPropertyRowMapper<StockTransTxLogCommand>(StockTransTxLogCommand.class));
                    }
    ...
    public void generateRtnWh(StockTransApplication sta) {
        ...
        if (sta.getType().equals(StockTransApplicationType.VMI_TRANSFER_RETURN)) {
            ...
            List<Long> skuIdList = staDao.findNotGiftSkuByStaId(sta.getId(), new SingleColumnRowMapper<Long>(Long.class));
            if ((skuIdList == null || skuIdList.size() == 0) && (sh.getDefaultCode() != null && "CK".equals(sh.getDefaultCode()))) {
                return;
            }
            StockTransVoucher stv = stvDao.findStvByStaId(sta.getId());
            List<Carton> cList = cartonDao.findCartonByStaId(sta.getId());// 通过STA查询箱号数据
            ...
            if (cList.size() > 0) {
                ...
                for (Carton c : cList) {
                    ...
                    for (CartonLine cl : clList) {
                        ...
                        if (sku.getIsGift() && (sh.getDefaultCode() != null && "CK".equals(sh.getDefaultCode()))) {
                            continue;
                        }
    ...
    public void generateReceivingTransfer(StockTransApplication sta) {
        ...
        if (sta.getType() == StockTransApplicationType.VMI_OWNER_TRANSFER) {
            ...
            List<Long> skuIdList = staDao.findNotGiftSkuByStaId(sta.getId(), new SingleColumnRowMapper<Long>(Long.class));
            if ((skuIdList == null || skuIdList.size() == 0) && (shop.getDefaultCode() != null && "CK".equals(shop.getDefaultCode()))) {
                return;
            }
            VmiTfxDefault tfx = new VmiTfxDefault();// 转店退仓反馈表
            tfx.setCreateTime(new Date());
            ...
            for (StvLineCommand out : outList) {
                ...
                if (sku.getIsGift() && (shop.getDefaultCode() != null && "CK".equals(shop.getDefaultCode()))) {
                    continue;
                }
