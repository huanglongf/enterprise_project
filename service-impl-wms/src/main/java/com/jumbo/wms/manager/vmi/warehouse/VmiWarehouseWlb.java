...
public class VmiWarehouseWlb extends AbstractVmiWarehouse {
    ...
    public boolean cancelSalesOutboundSta(String staCode, MsgOutboundOrderCancel msg) {
        ...
        // 如果未给外包仓直接标记为取消,取消单据
        if (order != null && (order.getStatus().equals(DefaultStatus.CREATED) || order.getStatus().equals(DefaultStatus.CANCELED))) {
