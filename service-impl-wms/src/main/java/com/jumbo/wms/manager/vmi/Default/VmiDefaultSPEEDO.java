 ...
 * 
 */
@Service("vmiDefaultSpeedo")
public class VmiDefaultSPEEDO extends BaseVmiDefault {
            ...
            for (int j = 0; j < qty.length; j++) {
                ...
                if (c == true) {
                    if (Long.parseLong(qty[2]) != 0L) {
                        ...
                        v.setVmiSource(bi.getVmiSource());
                        v.setUpc(sku.getExtensionCode2());
                        v.setQty(Long.parseLong(qty[2]));// 良品 占用库存总和
                        v.setInvStatus("残次品");
                        vmiInventoryDao.save(v);
                        c = false;
                    }
    ...
    public List<InventoryCommand> findVmiInventoryByOwner(List<String> ownerList) {
        return inventoryDao.findVmiInventoryByOwnerToSPEEDO(ownerList, new BeanPropertyRowMapper<InventoryCommand>(InventoryCommand.class));
    }


    /**
     * 获取批次号插入staline.ExtMemo
     */
    @Override
