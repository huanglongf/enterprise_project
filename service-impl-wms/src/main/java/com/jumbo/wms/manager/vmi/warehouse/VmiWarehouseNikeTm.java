...
public class VmiWarehouseNikeTm extends AbstractVmiWarehouse {
    /**
     * 
     */
    ...
    protected static final Logger log = LoggerFactory.getLogger(VmiWarehouseNikeTm.class);
    ...
    public boolean cancelSalesOutboundSta(String staCode, MsgOutboundOrderCancel msg) {
        ...
        try {
            log.debug("LF CaneleOrderToLFTask..." + Constants.VIM_WH_SOURCE_NIKE_IDS_TM);
            // 调用LF接口，获取取消结果
            ConfirmResult result = idsManagerProxy.siginOrderCancelResponseToLF(msg, Constants.VIM_WH_SOURCE_NIKE_IDS_TM);
            if (result != null) {
