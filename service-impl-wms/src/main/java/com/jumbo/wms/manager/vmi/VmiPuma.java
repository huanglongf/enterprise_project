...
public class VmiPuma extends VmiBaseBrand {

    private static final long serialVersionUID = -5836375961555155577L;

    private static final String invName = "良品";

    protected static final Logger log = LoggerFactory.getLogger(VmiPuma.class);
    @Autowired
    ...
    public void generateInboundStaSetHeadDefault(String slipCode, String vmisource, StockTransApplication head) {
        ...
        if (list.size() > 0) {
            ...
            head.setPumaType(list.get(0).getType());
    ...
    public void generateRtnWh(StockTransApplication sta) {
        ...
        if (sta.getType().equals(StockTransApplicationType.VMI_RETURN)) {
            ...
            if (!StringUtil.isEmpty(sh.getOpType())) {
                boolean b = checkVmiOpTypeReturn(sh.getOpType(), VmiOpType.RTW);
                if (b) {
            ...
            if (cList.size() > 0) {
                // 如果有箱号数据反馈箱号数据
                Map<String, CartonLineCommand> map = new HashMap<String, CartonLineCommand>();
                List<CartonLineCommand> cartonLineCommandList = cartonLineDao.findCartonLineGroupByCarId(sta.getId(), new BeanPropertyRowMapper<CartonLineCommand>(CartonLineCommand.class));
                if (null != cartonLineCommandList && cartonLineCommandList.size() > 0) {
                    for (CartonLineCommand list : cartonLineCommandList) {
                        if (map.get(list.getSkuBarcode()) == null) {
                            map.put(list.getSkuBarcode(), list);
                        } else {
                            CartonLineCommand l = map.get(list.getSkuBarcode());
                            l.setQty(l.getQty() + list.getQty());
                            map.put(list.getSkuBarcode(), l);
                        }
                    }
                }
                List<VmiRtoCommand> rtoList = vmiRtoDao.findVmiRtoByOrder(sh.getVmiCode(), sta.getRefSlipCode(), new BeanPropertyRowMapper<VmiRtoCommand>(VmiRtoCommand.class));
                Long rtoId = null;
                ...
                for (VmiRtoLineCommand list : lineList) {
                    VmiRtwLineDefault rtwL = new VmiRtwLineDefault();
                    if (map.containsKey(list.getUpc())) {
                        rtwL.setRtwId(rtw);
                        CartonLineCommand c = (CartonLineCommand) map.get(list.getUpc());
                        rtwL.setUpc(list.getUpc());
                        // 通过stvid 和 skuid查询 对应stvline中商品的状态
                        Long invS = stvDao.getInvStatus(stv.getId(), c.getSkuId(), new SingleColumnRowMapper<Long>(Long.class));
                        if (invS == null) {
                        ...
                        if (i.getName().equals(invName)) {
                            rtwL.setInvStatus("1");
                        } else {
                            rtwL.setInvStatus("0");
                        }
                        if (c.getQty().longValue() > list.getQty().longValue()) {
                            rtwL.setQty(list.getQty());
                            c.setQty(c.getQty() - list.getQty());
                        } else if (c.getQty().longValue() == list.getQty().longValue()) {
                            rtwL.setQty(list.getQty());
                            c.setQty(c.getQty() - list.getQty());
                        } else if (c.getQty().longValue() < list.getQty().longValue()) {
                            rtwL.setQty(c.getQty().longValue() > 0 ? c.getQty() : 0L);
                            c.setQty(c.getQty() - list.getQty());
                        }
                    } else {
                        rtwL.setUpc(list.getUpc());
                    ...
                    if (null != vmiBrandExt) {
                        ...
                        if (Constants.PUMA_VMI_CODE.equals(sh.getVmiCode())) { // 不同的品牌根据业务可传递不同的参数
                            SkuCommand skuCmd = new SkuCommand();
                            skuCmd.setExtensionCode2(list.getUpc());
                            extParam.setSkuCmd(skuCmd);
                            extParam.setShopCmd(sh);
                            extParam.setSta(sta);
                        }
                    ...
                    rtwL.setLineNo(list.getLineNo());
                    rtwL.setRtwId(rtw);
                    vmiRtwLineDao.save(rtwL);
                }

                /*
                 * for (Carton c : cList) { List<CartonLine> clList =
                 * cartonDao.findCartonLineByCarId(c.getId());// 查询箱号下的明细 for (CartonLine cl :
                 * clList) { Sku sku = skuDao.getByPrimaryKey(cl.getSku().getId()); if (sku == null)
                 * { throw new BusinessException(ErrorCode.SYSTEM_ERROR); } String upc =
                 * sku.getExtensionCode2(); if (StringUtil.isEmpty(upc)) { throw new
                 * BusinessException(ErrorCode.SYSTEM_ERROR); } // 通过stvid 和 skuid查询 对应stvline中商品的状态
                 * Long invS = stvDao.getInvStatus(stv.getId(), sku.getId(), new
                 * SingleColumnRowMapper<Long>(Long.class)); if (invS == null) { throw new
                 * BusinessException(ErrorCode.SYSTEM_ERROR); } InventoryStatus i =
                 * inventoryStatusDao.getByPrimaryKey(invS); if (i == null) { throw new
                 * BusinessException(ErrorCode.SYSTEM_ERROR); } VmiRtwLineDefault rtwL = new
                 * VmiRtwLineDefault(); rtwL.setUpc(upc); rtwL.setQty(cl.getQty());
                 * rtwL.setCartonNo(c.getCode());// 箱号 if (i.getName().equals(invName)) {
                 * rtwL.setInvStatus("1"); } else { rtwL.setInvStatus("0"); } rtwL.setRtwId(rtw);
                 * rtwL.setVersion(1); if (null != vmiBrandExt) { ExtParam extParam = new
                 * ExtParam(); if (Constants.PUMA_VMI_CODE.equals(sh.getVmiCode())) {//
                 * 不同的品牌根据业务可传递不同的参数 SkuCommand skuCmd = new SkuCommand();
                 * skuCmd.setExtensionCode2(sku.getExtensionCode2()); extParam.setSkuCmd(skuCmd);
                 * extParam.setShopCmd(sh); extParam.setSta(sta); }
                 * vmiBrandExt.generateRtnWhRtwLineAspect(rtwL, extParam); }
                 * vmiRtwLineDao.save(rtwL); } }
                 */
            } else {
                ...
                List<StockTransTxLogCommand> logList = stvLogDao.findStaLogByStvId(stv.getId(), new BeanPropertyRowMapper<StockTransTxLogCommand>(StockTransTxLogCommand.class));
                // 跟品牌给的指令对比
                Map<String, StockTransTxLogCommand> map = new HashMap<String, StockTransTxLogCommand>();
                List<VmiRtoCommand> rtoList = vmiRtoDao.findVmiRtoByOrder(sh.getVmiCode(), sta.getRefSlipCode(), new BeanPropertyRowMapper<VmiRtoCommand>(VmiRtoCommand.class));
                Long rtoId = null;
                VmiRtoCommand rto = null;
                // 校验指令数据
                ...
                for (StockTransTxLogCommand log : logList) {
                    ...
                    if (map.get(upc) == null) {
                        ...
                        StockTransTxLogCommand l = map.get(upc);
                        // l.setQuantity(l.getQuantity() + log.getQuantity());
                        l.setInQty(l.getInQty() + log.getInQty());
                        map.put(upc, l);
                    }
                }
                List<VmiRtoLineCommand> lineList = vmiRtoLineDao.findRtoLineListByRtoId(rtoId, new BeanPropertyRowMapper<VmiRtoLineCommand>(VmiRtoLineCommand.class));
                ...
                for (VmiRtoLineCommand list : lineList) {
                    ...
                    if (map.containsKey(list.getUpc())) {
                        rtwL.setUpc(list.getUpc());
                        StockTransTxLogCommand stlog = (StockTransTxLogCommand) map.get(list.getUpc());
                        InventoryStatus i = inventoryStatusDao.getByPrimaryKey(Long.parseLong(stlog.getInvStatus()));
                        if (i == null) {
                            throw new BusinessException(ErrorCode.SYSTEM_ERROR);
                        }
                        if (i.getName().equals(invName)) {
                        ...
                        if (stlog.getInQty().longValue() > list.getQty().longValue()) {
                            rtwL.setQty(list.getQty());
                            stlog.setInQty(stlog.getInQty() - list.getQty());
                        } else if (stlog.getInQty().longValue() == list.getQty().longValue()) {
                            rtwL.setQty(list.getQty());
                            stlog.setInQty(stlog.getInQty() - list.getQty());
                        } else if (stlog.getInQty().longValue() < list.getQty().longValue()) {
                            rtwL.setQty(stlog.getInQty().longValue() > 0 ? stlog.getInQty() : 0L);
                            stlog.setInQty(stlog.getInQty() - list.getQty());
                        }
                ...
                /*
                 * for (StockTransTxLogCommand log : logList) { Sku sku =
                 * skuDao.getByPrimaryKey(Long.parseLong(log.getSkuCode())); if (sku == null) {
                 * throw new BusinessException(ErrorCode.SYSTEM_ERROR); } String upc =
                 * sku.getExtensionCode2(); if (StringUtil.isEmpty(upc)) { throw new
                 * BusinessException(ErrorCode.SYSTEM_ERROR); } InventoryStatus i =
                 * inventoryStatusDao.getByPrimaryKey(Long.parseLong(log.getInvStatus())); if (i ==
                 * null) { throw new BusinessException(ErrorCode.SYSTEM_ERROR); } VmiRtwLineDefault
                 * rtwL = new VmiRtwLineDefault(); rtwL.setUpc(upc); rtwL.setQty(log.getInQty()); if
                 * (i.getName().equals(invName)) { rtwL.setInvStatus("1"); } else {
                 * rtwL.setInvStatus("0"); } rtwL.setRtwId(rtw); rtwL.setVersion(1); if (null !=
                 * vmiBrandExt) { ExtParam extParam = new ExtParam(); if
                 * (Constants.PUMA_VMI_CODE.equals(sh.getVmiCode())) {// 不同的品牌根据业务可传递不同的参数
                 * SkuCommand skuCmd = new SkuCommand();
                 * skuCmd.setExtensionCode2(sku.getExtensionCode2()); extParam.setSkuCmd(skuCmd);
                 * extParam.setShopCmd(sh); extParam.setSta(sta); }
                 * vmiBrandExt.generateRtnWhRtwLineAspect(rtwL, extParam); }
        ...
        if (sta.getType().equals(StockTransApplicationType.VMI_TRANSFER_RETURN)) {
            ...
            BiChannelCommand sh = biCannelDao.findVmiDefaultTbiChannelByOwen(sta.getOwner(), new BeanPropertyRowMapper<BiChannelCommand>(BiChannelCommand.class));
            if (!StringUtil.isEmpty(sh.getOpType())) {
                boolean b = checkVmiOpTypeReturn(sh.getOpType(), VmiOpType.TFX);
                if (b) {
                    // 不需要生成VMI反馈数据直接return
                    return;
                }
     ...
     * 验证VMI操作权限
     */
    public static boolean checkVmiOpType(String opType, VmiOpType type) {
        boolean b = true;
