...
public class VmiWarehouseGodivaHavi extends AbstractVmiWarehouse {

    /**
     * 
     */
    private static final long serialVersionUID = -3330829912481219232L;
    protected static final Logger log = LoggerFactory.getLogger(VmiWarehouseGodivaHavi.class);
    @Autowired
    ...
    public boolean cancelSalesOutboundSta(String staCode, MsgOutboundOrderCancel msg) {
        ...
        try {
            log.debug("LF CaneleOrderToLFTask..." + Constants.VIM_WH_SOURCE_GODIVA_HAVI);
            // 调用LF接口，获取取消结果
            ConfirmResult result = idsManagerProxy.siginOrderCancelResponseToLF(msg, Constants.VIM_WH_SOURCE_GODIVA_HAVI);
            if (result != null) {
    ...
    public boolean cancelReturnRequest(Long msgLog) {
        ...
        // 查找外包仓服务信息
        IdsServerInformation idsServerInformation = idsServerInformationDao.findServerInformationBySource(Constants.VIM_WH_SOURCE_GODIVA_HAVI);
        // 将退货信息生成指定格式
