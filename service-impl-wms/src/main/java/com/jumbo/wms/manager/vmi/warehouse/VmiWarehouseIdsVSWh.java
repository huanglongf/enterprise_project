...
public class VmiWarehouseIdsVSWh extends AbstractVmiWarehouse {
    ...
    public boolean cancelSalesOutboundSta(String staCode, MsgOutboundOrderCancel msg) {
        ChooseOption chooseOption = chooseOptionDao.findByCategoryCodeAndValue("IDS", "cancelSalesOutboundSta");
        if ("0".equals(chooseOption.getOptionKey())) {
            ...
            if (order != null && order.getStatus().equals(DefaultStatus.CREATED)) {
                // msgOutboundOrderDao.updateStatusByStaCode(DefaultStatus.FINISHED.getValue(),
                // staCode);
            ...
            try {
                // 调用LF接口，获取取消结果
                ConfirmResult result = idsManagerProxy.siginOrderCancelResponseToLF(msg, Constants.VIM_WH_SOURCE_IDS_VS);
                if (result != null) {
    ...
    public boolean cancelReturnRequest(Long msgLog) {
        ...
        if ("0".equals(chooseOption.getOptionKey())) {
            ...
            // 查找外包仓服务信息
            IdsServerInformation idsServerInformation = idsServerInformationDao.findServerInformationBySource(Constants.VIM_WH_SOURCE_IDS_VS);
            // 将退货信息生成指定格式
