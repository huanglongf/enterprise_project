...
public class VmiWarehouseAEO extends AbstractVmiWarehouse {
    ...
    protected static final Logger logger = LoggerFactory.getLogger(VmiWarehouseAEO.class);
    ...
    public boolean cancelSalesOutboundSta(String staCode, MsgOutboundOrderCancel msg) {
        ...
        try {
            log.debug("LF CaneleOrderToLFTask..." + Constants.VIM_WH_SOURCE_AEO_IDS);
            // 调用LF接口，获取取消结果
            ConfirmResult result = idsManagerProxy.siginOrderCancelResponseToLF(msg, Constants.VIM_WH_SOURCE_AEO_IDS);
            logger.info("AEO+" + staCode + result);
            if (result != null) {
    ...
    public boolean cancelReturnRequest(Long msgLog) {
        ...
        // 查找外包仓服务信息
        IdsServerInformation idsServerInformation = idsServerInformationDao.findServerInformationBySource(Constants.VIM_WH_SOURCE_AEO_IDS);
        // 将退货信息生成指定格式
        String respXml = idsManager.cancelReturnResponseLF(idsServerInformation, msgLog);
        if (StringUtils.hasText(respXml)) {
            try {
                logger.info("AEO cancel respXml :" + respXml);
                ...
                logger.info("AEO cancel result :" + resultXml);
