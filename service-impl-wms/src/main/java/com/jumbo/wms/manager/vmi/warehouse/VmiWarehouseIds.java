...
public class VmiWarehouseIds extends AbstractVmiWarehouse {
    ...
    public boolean cancelSalesOutboundSta(String staCode, MsgOutboundOrderCancel msg) {
        ...
        if ("0".equals(chooseOption.getOptionKey())) {
            ...
            try {
                // 调用LF接口，获取取消结果
                ConfirmResult result = idsManagerProxy.siginOrderCancelResponseToLF(msg, Constants.VIM_WH_SOURCE_IDS);
                if (result != null) {
    ...
    public boolean cancelReturnRequest(Long msgLog) {
        ...
        if ("0".equals(chooseOption.getOptionKey())) {
            ...
            // 查找外包仓服务信息
            IdsServerInformation idsServerInformation = idsServerInformationDao.findServerInformationBySource(Constants.VIM_WH_SOURCE_IDS);
            // 将退货信息生成指定格式
