...
import com.jumbo.InitSystemConfig;

public class DefaultWmsExcelReader implements ExcelReader, Serializable {
    ...
    static final Logger logger = LoggerFactory.getLogger(DefaultWmsExcelReader.class);
    ...
    public ReadStatus readAllPerSheet(InputStream is, Map<String, Object> beans) {
        ...
        try {
            ...
            if (logger.isErrorEnabled()) {
                logger.error("readAllPerSheet InterruptedException！", e);
            }
            readStatus.setStatus(ReadStatus.STATUS_SYSTEM_ERROR);
            readStatus.setMessage("New Instance Error");
        } catch (IllegalAccessException e) {
            if (logger.isErrorEnabled()) {
                logger.error("readAllPerSheet IllegalAccessException！", e);
            }
            readStatus.setStatus(ReadStatus.STATUS_SYSTEM_ERROR);
            readStatus.setMessage("New Instance Error");
        }
    ...
    private void readSheet(Workbook wb, int sheetNo, ExcelSheet sheetDefinition, OgnlStack stack, ReadStatus readStatus) {
        ...
        for (ExcelBlock blockDefinition : sheetDefinition.getExcelBlocks()) {
            if (((skipErrors && readStatus.getStatus() == ReadStatus.STATUS_DATA_COLLECTION_ERROR) || readStatus.getStatus() == ReadStatus.STATUS_SUCCESS) && (loopBlock < 1 || !blockDefinition.isLoop())) {
    ...
    private void readSimpleBlock(Workbook wb, int sheetNo, ExcelBlock blockDefinition, OgnlStack stack, ReadStatus readStatus) {
        ...
        for (ExcelCell cellDefinition : blockDefinition.getCells()) {
            ...
            try {
                ...
                value = checkValue(sheetNo, ExcelUtil.getCellIndex(cellDefinition.getRow(), cellDefinition.getCol()), value, cellDefinition, getPropertyType(stack.peek(), cellDefinition));
                ...
                if (readStatus.getStatus() == ReadStatus.STATUS_SUCCESS) readStatus.setStatus(ReadStatus.STATUS_DATA_COLLECTION_ERROR);
                readStatus.addException(e);
            } catch (Exception e) {
                if (logger.isErrorEnabled()) {
                    logger.error("readSimpleBlock Exception！", e);
                }
                readStatus.setStatus(ReadStatus.STATUS_SYSTEM_ERROR);
    ...
    private void readLoopBlock(Workbook wb, int sheetNo, ExcelBlock blockDefinition, OgnlStack stack, ReadStatus readStatus) {
        ...
        try {
            ...
            if (logger.isErrorEnabled()) {
                logger.error("readLoopBlock Exception！", e);
            }
            readStatus.setStatus(ReadStatus.STATUS_SYSTEM_ERROR);
            readStatus.setMessage(e.getMessage());
        }
    ...
    private Object readBlock(Workbook wb, int sheetNo, ExcelBlock blockDefinition, int startRow, ReadStatus readStatus) throws Exception {
        Sheet sheet = wb.getSheetAt(sheetNo);
        if (sheet.getLastRowNum() > InitSystemConfig.getExcelRowsLimit()) {
            throw new BusinessException(ErrorCode.WRONG_DATA_FORMAT, "文件行数超出限制：" + InitSystemConfig.getExcelRowsLimit() + " 行,当前行数：" + sheet.getLastRowNum());
        }
        FormulaEvaluator evaluator = wb.getCreationHelper().createFormulaEvaluator();
        ...
        if (blockDefinition.getLoopClass() == null) {
            ...
            for (ExcelCell cellDefinition : blockDefinition.getCells()) {
                ...
                try {
                    Object value = getCellValue(cell, evaluator);
                    value = checkValue(sheetNo, ExcelUtil.getCellIndex(startRow + rowOffSet, cellDefinition.getCol()), value, cellDefinition, getPropertyType(result, cellDefinition));
    ...
    private Object checkValue(int sheetNo, String cellIndex, Object value, ExcelCell cellDefinition, Class<? extends Object> clazz) throws ExcelManipulateException {
        ...
        if (dc == null) {
            throw new ExcelManipulateException(ErrorCode.UNSUPPORTING_DATA_TYPE, new Object[] {sheetNo + 1, cellIndex, null, cellDefinition.getPattern(), cellDefinition.getChoiceString()});
        }
    ...
    private Object getCellValue(Cell cell, FormulaEvaluator evaluator) throws ExcelManipulateException {
        ...
        switch (cellValue.getCellType()) {
            ...
            case Cell.CELL_TYPE_ERROR:
                logger.warn("{}: Error with formula value!", ExcelUtil.getCellIndex(cell.getRowIndex(), cell.getColumnIndex()));
                break;
    ...
    private boolean checkBreak(Sheet sheet, int row, int col, LoopBreakCondition condition) {
        ...
        if (sheet.getLastRowNum() < row) return true;
        if (condition != null) {
            ...
            if (condition.getFlagString().equals(cell.getRichStringCellValue().getString())) return true;
        }
        return false;
    }
