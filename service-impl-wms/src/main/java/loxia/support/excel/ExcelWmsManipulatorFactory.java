...
import loxia.support.excel.impl.DefaultWmsExcelReader;
...
public class ExcelWmsManipulatorFactory {
    static final Logger logger = LoggerFactory.getLogger(ExcelWmsManipulatorFactory.class);
    ...
    public void setConfig(String... configurations) {
        for (String config : configurations) {
            Digester digester = DigesterLoader.createDigester(new InputSource(Thread.currentThread().getContextClassLoader().getResourceAsStream(RULE_FILE)));
            digester.setValidating(false);
            try {
                List<ExcelSheet> list = (List<ExcelSheet>) digester.parse(Thread.currentThread().getContextClassLoader().getResourceAsStream(config));
                for (ExcelSheet es : list)
                    sheetDefinitions.put(es.getName(), es);
            } catch (IOException e) {
                if (logger.isErrorEnabled()) {
                    logger.error("setConfig Exception！", e);
                }
                throw new RuntimeException("Read excel config failed.");
            } catch (SAXException e) {
                if (logger.isErrorEnabled()) {
                    logger.error("setConfig SAXException！", e);
                }
                throw new RuntimeException("Read excel config failed.");
            }
        }
    ...
    public ExcelReader createExcelReader(Class<? extends ExcelReader> clazz, String... sheets) {
        ...
        if (clazz == null)
            excelReader = new DefaultWmsExcelReader();
        else {
            try {
    ...
    private ExcelSheet getExcelSheet(String sheet) {
        ...
        if (sheetDefinition == null) throw new RuntimeException("No sheet defintion found with name: " + sheet);
