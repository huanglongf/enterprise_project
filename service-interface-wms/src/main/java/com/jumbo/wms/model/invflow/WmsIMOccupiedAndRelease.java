...
/**
 * WMS3.0通知IM的无前置单号的占用数据和取消释放数据中间表
 *
 */
@Entity
@Table(name = "T_WH_IM_OCCUPIED_RELEASE")
@org.hibernate.annotations.Entity(optimisticLock = OptimisticLockType.VERSION)
public class WmsIMOccupiedAndRelease extends BaseModel {
    ...
    /**
     * 推送状态，0：新建；1:已推送; 20:不推送
     */
    private Integer status = 0;

    @Id
    @Column(name = "ID")
    @SequenceGenerator(name = "SEQ_IM_OCCUPIED_RELEASE", sequenceName = "S_T_WH_IM_OCCUPIED_RELEASE", allocationSize = 1)
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "SEQ_IM_OCCUPIED_RELEASE")
    public Long getId() {
        return Id;
    }
