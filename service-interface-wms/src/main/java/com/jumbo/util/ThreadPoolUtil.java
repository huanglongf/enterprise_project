...
public class ThreadPoolUtil {
    ...
    public static ThreadPoolExecutor getPool(int threadCount) {
        ExecutorService pool = new ThreadPoolExecutor(threadCount, threadCount,  0L, TimeUnit.MILLISECONDS,  new LinkedBlockingQueue<Runnable>());  
        ...
        try {
            while (true) {
                ...
                if (todoTotal >= threadCount) {
                    try {
                        System.out.println(todoTotal + "---" + threadCount);
                        Thread.sleep(500L);
