package com.jumbo.webservice.pda.getWarehouses;

import java.io.Serializable;

import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlElement;
import javax.xml.bind.annotation.XmlType;

import com.jumbo.webservice.base.AuthRequest;
import com.jumbo.webservice.base.AuthRequestHeader;
import com.jumbo.webservice.pda.BaseRequestBody;


/**
 * <p>
 * getWarehousesRequest complex type的 Java 类。
 * 
 * <p>
 * 以下模式片段指定包含在此类中的预期内容。
 * 
 * <pre>
 * &lt;complexType name="getWarehousesRequest">
 *   &lt;complexContent>
 *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *       &lt;sequence>
 *         &lt;element name="authRequestHeader" type="{http://webservice.jumbo.com/pda/}authRequestHeader"/>
 *         &lt;element name="baseRequestBody" type="{http://webservice.jumbo.com/pda/}baseRequestBody"/>
 *       &lt;/sequence>
 *     &lt;/restriction>
 *   &lt;/complexContent>
 * &lt;/complexType>
 * </pre>
 * 
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "getWarehousesRequest", propOrder = {"authRequestHeader", "baseRequestBody"})
public class GetWarehousesRequest implements AuthRequest, Serializable {

    /**
     * 
     */
    private static final long serialVersionUID = 2609954256526894191L;
    @XmlElement(required = true)
    protected AuthRequestHeader authRequestHeader;
    @XmlElement(required = true)
    protected BaseRequestBody baseRequestBody;

    /**
     * 获取authRequestHeader属性的值。
     * 
     * @return possible object is {@link AuthRequestHeader }
     * 
     */
    @Override
    public AuthRequestHeader getAuthRequestHeader() {
        return authRequestHeader;
    }

    /**
     * 设置authRequestHeader属性的值。
     * 
     * @param value allowed object is {@link AuthRequestHeader }
     * 
     */
    public void setAuthRequestHeader(AuthRequestHeader value) {
        this.authRequestHeader = value;
    }

    /**
     * 获取baseRequestBody属性的值。
     * 
     * @return possible object is {@link BaseRequestBody }
     * 
     */
    public BaseRequestBody getBaseRequestBody() {
        return baseRequestBody;
    }

    /**
     * 设置baseRequestBody属性的值。
     * 
     * @param value allowed object is {@link BaseRequestBody }
     * 
     */
    public void setBaseRequestBody(BaseRequestBody value) {
        this.baseRequestBody = value;
    }

}
