...
<script>
	$(function(){
		$("#table_split_content tbody tr").dblclick(function(){
			toShowAssociationSplit();
		});
	});
</script>
<div>
<!-- 
	...
	<div style="overflow:auto; border:solid #F2F2F2 1px;">
		<table id="table_split_content" class="table table-hover table-striped no-margin">
	   		<thead>
			...
			<tbody align="center">
		  		<c:forEach items="${split_list }" var="res">
		  			<tr style="cursor:pointer;">
			  			<td class="td_cs" title="${res.wo_no }">${res.wo_no }</td>
			  			<td class="td_cs" title="${res.create_by_display }">${res.create_by_display }</td>
			  			<td class="td_cs" title="${res.split_to }">${res.split_to }</td>
			  			<td class="td_cs" title="${res.wo_status }">${func:getDictLabel('wo_store_master_wo_status', res.wo_status) }</td>
			  		</tr>
		  		</c:forEach>
	  		</tbody>
		</table>
