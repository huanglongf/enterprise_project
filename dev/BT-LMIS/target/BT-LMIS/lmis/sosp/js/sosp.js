		...
		success : function(result) {
			if(result.result_code=="SUCCESS"){
				...
				for(var i =0;i < result.manECListShow.length;i++){
					manEC += 
						'<tr><td>'　+　result.manECListShow[i].no +　'</td>'
						+ '<td>' +　result.manECListShow[i].ladder_type + '</td>'
						+ '<td>' +　result.manECListShow[i].region + '</td>'
						+ '<td>' +　result.manECListShow[i].charge_percent + '</td>'
...
function del_manEC(id){
	...
	$.ajax({
		url : root + "/control/expressContractController/delManEC.do",
		type : "post",
		data : {
			"id" : id,
			"con_id" : con_id,
		...
		success : function(result) {
			if(result.result_code=="SUCCESS"){
				alert(result.result_content);
				returnManagement();
				$("#manECList tbody").empty();
				var manEC = "";
				for(var i =0;i < result.manECListShow.length;i++){
					manEC += 
						'<tr><td>'　+　result.manECListShow[i].no +　'</td>'
						+ '<td>' +　result.manECListShow[i].carrier + '</td>'
						+ '<td>' +　result.manECListShow[i].com + '</td>'
						+ '<td>' +　result.manECListShow[i].ladder_type + '</td>'
						+ '<td>' +　result.manECListShow[i].region + '</td>'
						+ '<td>' +　result.manECListShow[i].charge_percent + '</td>'
						+ '<td><a onclick="del_manEC(' + result.manECListShow[i].id +')" >删除</a></td>'
						+'</tr>';
				}
				$("#manECList tbody").append(manEC);
...
function saveManagementEC(){
	var con_id = $("#id").val(); 
	if (con_id == 0 || con_id == null){
		alert("无对应合同主信息！");
		return false;
	}
	var carrierUsed = $("#carrierUsed_man").val();
	if(carrierUsed == ""){
		alert("未维护对应承运商！");
		return false;
	}
	var freight = isChecked("cb_8");
	var insurance = isChecked("cb_9");
	var special_service = isChecked("cb_10");
	var cod = isChecked("cb_11");
	var delegated_pickup = isChecked("cb_12");
	if(freight == false && insurance == false && cod == false && delegated_pickup == false){
		alert("请至少选择一条收费项！");
		return false;
	}
	if(special_service == true){
		if(cod == false && delegated_pickup == false){
			alert("请至少选择一项特殊服务费！");
			return false;
		}
	}
	var ladder_type = $("#ladder_type_man").val();
	var json = {
			"con_id": con_id, 
			"belong_to": carrierUsed, 
			"freight": freight, 
			"insurance": insurance, 
			...
			"ladder_type": ladder_type}
	if(ladder_type != 1) {
		var num_1 = $("#payAll_1_man").val();
		var num_2 = $("#payAll_2_man").val();
		if(num_1　==　"" && num_2　==　""){
			alert("请设定区间！");
			return false;
		}
		...
		if(num_2 == ""){
			json["compare_1"]=$("#compare_1_man").val();
			json["num_1"]=num_1;
			num_2 = num_1 + 1;
			json["compare_2"]="2";
		} else {
			json["compare_1"]=$("#compare_1_man").val();
			json["num_1"]=num_1;
			json["compare_2"]=$("#compare_2_man").val();
			json["num_2"]=num_2;
		}
		if(parseInt(num_1) > parseInt(num_2)){
			alert("区间赋值异常：下限大于上限！");
		...
		json["uom_1"] = $("#payAll_1_uom_man").val();
		json["rel"] = $("#rel_man").val();
		json["uom_2"] = $("#payAll_2_uom_man").val();
	}
	var charge_percent = $("#charge_percent_man").val();
	if(charge_percent == ""){
		alert("收费率未填写！");
		return false;
	}
	...
	json["charge_percent"] = charge_percent;
	json["charge_percent_uom"] = $("#charge_percent_uom_man").val();
	$.ajax({
		url: root +　"/control/expressContractController/saveManEC.do",
		type:　"post",
		data: json,
		dataType:　"json",
		success:　function(result){
...
function managementShow(){
	if($("#cb_managementFee").prop("checked")==true){
		$("#managementFee").show();
		var con_id = $("#id").val();
		if(con_id != ""){
			$.ajax({
				url : root +　"/control/expressContractController/loadManEC.do",
				type :　"post",
				data : {
					"con_id":con_id,
					"belong_to" : $("#contractOwner").val()
...
function managementOtherShow(obj){
	if($("#cb_managementFee_other_" + obj).prop("checked")==true){
		$("#managementFeeOther_" + obj).show();
...
function initializeManagement(){
	$("#charge_percent_man").val("");
	$("#charge_percent_uom_man option:eq(0)").prop("selected", "selected");
	$("#compare_1_man option:eq(0)").prop("selected", "selected");
	$("#payAll_1_man").val("");
	$("#payAll_1_uom_man option:eq(0)").prop("selected", "selected");
	$("#rel_man option:eq(0)").prop("selected", "selected");
	$("#compare_2_man option:eq(0)").prop("selected", "selected");
	$("#payAll_2_man").val("");
	$("#payAll_2_uom_man option:eq(0)").prop("selected", "selected");
	$("#ladder_type_man option:eq(0)").prop("selected", "selected");
	$("#cb_11").attr("checked",false);
	$("#cb_12").attr("checked",false);
	$("#cb_8").attr("checked",false);
	$("#cb_9").attr("checked",false);
	$("#cb_10").attr("checked",false);
	$("#carrierUsed_man option:eq(0)").prop("selected", "selected");
}
...
function shiftPage_5Other(){
	...
	if ($("#ladder_type_man_other").val() == 3) {
		$("#div_10_other").show();
		$("#div_11_other").show();
	}
}

function shiftPage_5(){
	if ($("#ladder_type_man").val() == 0) {
		$("#div_10").hide();
		$("#div_11").hide();
	}
	if ($("#ladder_type_man").val() == 1) {
		$("#div_10").show();
		$("#div_11").hide();
	}
	if ($("#ladder_type_man").val() == 2) {
		$("#div_10").show();
		$("#div_11").show();
	}
	if ($("#ladder_type_man").val() == 3) {
		$("#div_10").show();
		$("#div_11").show();
	}
}
