 ...
 * @returns
 */
function tableconfig(tableFunctionConfig) {
	// 表名赋值
	$("#table_name").val(tableFunctionConfig.tableName)
	// 初始化配置
	initTableConfig(tableFunctionConfig.tableName);
	// 弹窗
...
function countTableColumn() {
	...
	$("#table_column_content").parent().parent().next().text("字段总量"+$("#table_column_content table tbody").children().length+"条，已选"+choosen+"条");
}
