...
<!-- 				    	</td> -->
					...
					<td align="center" width="10%">
						<button class="btn btn-xs btn-yellow" onclick="back_to_main()">
...
<table class="table table-striped" style="table-layout: fixed;">	 		
	...
	<script type="text/javascript">
	  	function pageJump() {
	  		 openDiv(root+'/orderController/import_main.do?'+data+'&startRow='+$("#startRow").val()+'&endRow='+$("#startRow").val()+"&page="+$("#pageIndex").val()+"&pageSize="+$("#pageSize").val());
	  	}
		function toAdd(){
			 openDiv('${root}/orderController/toUpload.do');
		}
		
		function detail(bat_id){
			 openDiv('${root}/control/addressController/toload.do?bat_id='+bat_id);
			}
		/**
	   	* 转换
	   	*/
	  	function transformation(){
	  		...
	  		var trans_url=root + "control/addressController/transformation.do";
	  		$.ajax({
	  			cache:false,
	  			type:'POST',
	  			url:trans_url,
		...
		function pageJump() {
	      openDiv('${root}importOrderController/load.do?startRow='+$("#startRow").val()+'&endRow='+$("#startRow").val()+"&page="+$("#pageIndex").val()+"&pageSize="+$("#pageSize").val(),'订单导入');
		}
	  	/**
	   	* 导入
	   	*/
	  	function imp(){
			...
			$.ajaxFileUpload({
				//上传地址
				url: root+'control/addressController/import.do',
				//是否需要安全协议，一般设置为false
	            secureuri: false, 
	          	//文件上传域的ID
				fileElementId: 'file1', 
				...
				success: function (data, status){
					alert(data.msg); 
					if(data.code == 200) {
						openDiv(root+'control/addressController/import_main.do','订单导入');
	                }
	  	...
	  	function deleteItem(){
	  	  ...
	  	  if (confirm("你确定要删除这些数据吗？")) {
	  	    var url=root+"control/addressController/deleteImport.do?record="+check_val;
	  			$.ajax({
	  				type : "POST",
	  				url: url,  
	  				data:"",
		...
		function back_to_main(){
			openDiv(root+"control/addressController/load.do");
		}
