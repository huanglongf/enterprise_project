...
<head lang="en">
	...
	<script type='text/javascript'>
			function submit(){
				...
				$.ajax({
					url : root + "control/radar/ageingMasterController/updateAgeingMaster.do",	
					type : "post",
			       	data : {
				   		"remark" :remark,
				   		"id" :id,
	...
	<div class= "col-xs-12" >
		<div class= "clearfix form-group no-margin-bottom no-padding-bottom no-border-bottom" >
			<div class= "no-margin-bottom no-padding-bottom no-border-bottom" style= "width: 100%;" align= "center" >
				<input id= "id" type= "hidden" value= "${queryParam.id }" />
						...
						<td class="left" nowrap="nowrap" width="15%">
							<select id="store" name="store" style="width: 100%;" data-edit-select="1">
								...
								<c:forEach items= "${store }" var= "store" >
									<option value= "${store.storeCode }" ${queryParam.storeCode == store.storeCode? "selected= 'selected'": "" } >
										${store.storeName }
									</option>
								</c:forEach>
							</select>
