	...
	<head lang= "en">
		...
		<script type= "text/javascript">
			...
			function find(){
				...
				openDiv("${root }control/mainController/b.do?container_code="+$("#container_code").val()+"&stime="+sdate+"&etime="+edate
							+"&type="+$("#type").val()
			...
			function export_excel(){
				...
				var edate = new Date(etime).getTime();
				 $.ajax({
					type : "POST",
					url: "${root}control/mainController/export_b.do",  
					data:"container_code="+$("#container_code").val()+"&stime="+stime+"&etime="+etime
					+"&type="+$("#type").val()
					+"&create_user="+$("#create_user").val()
					+"&location="+$("#location").val()
					+"&sku="+$("#sku").val(),
			...
			<div class= "page-header" style= "margin-left: 20px; margin-bottom: 20px;" >
				<table>
			  		<tr>
			  			...
			  			<td align= "right" width= "10%"><label class= "blue" for= "client_name">操作类型&nbsp;:</label></td>
			  			<td width= "20%">
							<select id="type" name="type">
								<option value="0" selected="selected">请选择</option>
								<option value="上架" <c:if test="${queryParam.type=='上架'}">selected="selected"</c:if>>上架</option>
								<option value="下架" <c:if test="${queryParam.type=='下架'}">selected="selected"</c:if>>下架</option>
							</select>
						</td>
			  		</tr>
			  		<tr>
			  			...
			  			<td align= "right" width= "10%"><label class= "blue" for= "client_code">货位号&nbsp;:</label></td>
			  			<td width= "20%"><input id= "location" name= "location" type= "text" value= "${queryParam.location }" /></td>
			...
			<table class= "table table-striped" style= "table-layout: fixed;" >
		   		<thead>
			  		<tr class= "table_head_line" >
			  			...
			  			<td class= "td_cs" >操作类型</td>
			  			<td class= "td_cs" >容器号</td>
			  			<td class= "td_cs" >货位号</td>
		  		...
		  		<tbody align= "center">
			  		<c:forEach items= "${pageView.records }" var= "res" >
				  		<tr>
				  			...
				  			<td class= "td_cs" >${res.type }</td>
				  			<td class= "td_cs" >${res.container_code }</td>
				  			<td class= "td_cs" >${res.location }</td>
