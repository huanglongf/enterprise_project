 ...
 *
 */
public class OperatingCostEstimateTest extends EstimateAbstract {
	...
	public void estimateBatchTest(){
		...
		for (int i = 0; i < cbList.size(); i++) {
			...
			param.put("logTitle","操作费预估");
			
//
//			estimateParam.setFromDate("2017-03-05 00:00:00");
//			estimateParam.setToDate("2017-04-05 23:59:59");

			estimateParam.setFromDate("2016-03-01");
			estimateParam.setToDate("2017-05-10");
			...
			//操作费预估主体方法，基于合同，查找对应规则，执行明细生成、明细汇总
			EstimateResult estimateResult = operatingCostEstimate.estimate(estimateParam);

			if(estimateResult.isFlag()){
				successNum++;
				messageList.add("uuid:" + estimateParam.getBatchNumber() + "操作费预估成功");
			} else {
				errorNum++;
				messageList.add("uuid:" + estimateParam.getBatchNumber() + "操作费预估异常");
			}
		}
	...
	public void estimateTest(){
		...
		ContractBasicinfo cb = contractBasicinfoService.findById(294);
		...
		if(estimateResult.isFlag()){
			successNum++;
			messageList.add("uuid:" + estimateParam.getBatchNumber() + "操作费预估成功"+estimateResult.getMsg().get("msg"));
		} else {
			errorNum++;
			messageList.add("uuid:" + estimateParam.getBatchNumber() + "操作费预估异常"+estimateResult.getMsg().get("msg"));
		}
