...
<script>
	 $(function(){
		$("#table_detail_content2 tbody tr").dblclick(function(){
			...
			$.ajax({
		        ...
		        dataType: "text",
		        data: {id:id,waybill:waybill,sel_flag:2,readonly:1},
		        success : function(data){
		        	$("#packages_form2").remove();
		        	$("body").append(data);
					$("#packages_form2").modal({backdrop: "static", keyboard: false});
					// 隐藏遮罩
					$(".modal-backdrop").hide();
		        }
			}); 
...
<div id="add_list">
	<div style="overflow:auto; border:solid #F2F2F2 1px;">
		<table id="table_detail_content2" class="table table-hover table-striped no-margin">
	   		<thead>
...
<div id="model_content">
	<jsp:include page="/work_order/wo_platform_store/package_list2.jsp" flush="true" />
</div>