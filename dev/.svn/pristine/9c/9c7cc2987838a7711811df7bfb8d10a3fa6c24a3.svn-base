...
public class ExcelExportUtil {
	 ...
	 * @throws Exception
	 */
	public static void exportExcelData(List<Map<String,Object>> list,String[] headNames, String sheetName,HSSFWorkbook book)  throws Exception {
		exportXlsHandle(list,headNames, sheetName,book);
	}
	
	
	/**
	 * 导出excel文件-支持2007 2010  xlsx扩展名
	 * @param list 需要写的数据
	 * @param headColums 数据的列名（字段名，数据里的key名 不能为null） 
	 * @param headNames 列名说明-一般是字段说明，（不需要时，可以传 null）
	 * @param sheetName sheet的名称
	 * @throws Exception
	 */
	public static void exportExcelData(List<Map<String,Object>> list,String[] headNames, String sheetName,XSSFWorkbook book)  throws Exception {
		exportXlsxHandle(list,headNames, sheetName,book);
	}
	 ...
	 * @throws Exception
	 */
	private static void exportXlsHandle(List<Map<String,Object>> list,String[] headNames , String sheetName,HSSFWorkbook book) throws Exception {
		HSSFSheet sheet = book.createSheet(sheetName);
		//当前行
		int curRow = 0;
	 ...
	 * @throws Exception
	 */
	private static void exportXlsxHandle(List<Map<String,Object>> list,String[] headNames , String sheetName,XSSFWorkbook book) throws Exception {
		XSSFSheet sheet = book.createSheet(sheetName);
		//当前行
		int curRow = 0;
	 ...
	 * @Author duoduo
	 */
	public static void exportNewShell(List<String[]> list,String rootDir,String fileName)  throws Exception {
		File file = new File(rootDir, fileName+".xlsx");
	 ...
	 * @param content 数据集
	 */
	public static <T> void export(OutputStream out, List<T[]> content) {
		final List<T[]> list = content;
		export(out, new Function<Integer, T[]>() {
			@Override
			public T[] apply(Integer rowIndex) {
	 ...
	 *
	 * @param out  输出流
	 * @param func 单行数据获取Fun,返回NULL跳出
	 */
	public static <T> void export(OutputStream out, Function<Integer, T[]> func) {
		// 内存中保留100行，超出flush到磁盘
