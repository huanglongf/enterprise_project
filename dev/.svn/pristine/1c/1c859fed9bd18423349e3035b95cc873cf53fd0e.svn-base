...
public class RobortD1 extends Thread {
	...
	public void run(){
		...
		try {
			if(tableContent0.size()!=0){
				File f=BigExcelExport.excelDownLoadDatab_Z(tableContent0, tableHeader, obj.getFileName(), "无差异-1"+"的.xlsx");	
			}
			if(tableContent.size()!=0){
				File f1=BigExcelExport.excelDownLoadDatab_Z(tableContent, tableHeader, obj.getFileName(), "有差异-1"+"的.xlsx");	
			}
			} catch (IOException e) {
	...
	public RobortD1(DifferenceObj obj) {
		super();
		this.obj = obj;
	}
}
