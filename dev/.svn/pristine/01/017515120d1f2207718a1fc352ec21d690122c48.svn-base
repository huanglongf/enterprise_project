		...
		$(document).ready(function(){
			$.fn.zTree.init($("#menuTree"), setting, zNodes);
	  		...
	  		$("#add").click(function(){
				...
				openDiv(root+"/control/lmis/areaController/findOneaddArea_form.do?id="+gId);
	  			 
			});
			$("#update").click(function(){
				if(gId==''){alert("请选择一项！");return;}
				openDiv(root+"/control/lmis/areaController/findOneupdateArea_form.do?id="+gId);
				...
				if(window.confirm('您确定要删除 "'+gName+'" 此节点吗？')){
					 $.ajax({  
				           ...
				           success: function (jsonStr) {  
				               ...
				               openDiv('/BT-LMIS//control/lmis/areaController/toForm.do');;
				           }  
				    }); 
				}
				/* */
