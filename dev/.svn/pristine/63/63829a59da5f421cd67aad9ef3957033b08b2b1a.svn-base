...
<script>
	$(function() {
		...
		$("#table_detail_content tbody tr").dblclick(function(){toProcessForm("process", $(this));});
		...
		<table class="table table-striped" style="table-layout:fixed;">
	   		<thead>
		  		<tr class="table_head_line">
		  			<td class="td_ch" ><input type="checkbox" id="checkAllTable" onclick="inverseCkb('ckb')" /></td>
					...
					<td class="td_cs" title="错漏发条形码">错漏发条形码</td>
					<td class="td_cs" title="错漏发商品数量">错漏发商品数量</td>
					<td class="td_cs" title="收件人">收件人</td>
					<td class="td_cs" title="联系电话">联系电话</td>
					<td class="td_cs" title="联系地址">联系地址</td>
		  		</tr>
			</thead>
		</table>
	</div>
	  		...
	  		<tbody align="center">
		  		<c:forEach items="${pageView.records }" var="res">
		  			<tr style="cursor:pointer;">
			  			...
			  			<td class="td_cs" title="${res.query_person }（双击进入）">${res.query_person }</td>		<td class="td_cs" title="${res.related_number }（双击进入）">${res.related_number }</td>
						...
						<td class="td_cs" title="${res.mistake_barcode }（双击进入）">${res.mistake_barcode }</td>
						<td class="td_cs" title="${res.mistake_product_count }（双击进入）">${res.mistake_product_count }</td>
						<td class="td_cs" title="${res.recipient }（双击进入）">${res.recipient }</td>
						<td class="td_cs" title="${res.phone }（双击进入）">${res.phone }</td>
						<td class="td_cs" title="${res.address }（双击进入）">${res.address }</td>
			  		</tr>
			  	</c:forEach>
	  		</tbody>
		</table>
