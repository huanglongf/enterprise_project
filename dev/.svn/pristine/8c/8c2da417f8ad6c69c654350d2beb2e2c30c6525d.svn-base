 ...
 *
 */
public class ConsumableCostEstimateTest extends EstimateAbstract {
...
//			estimateParam.setToDate("2017-04-05 23:59:59");
			...
			EstimateResult estimateResult = consumableCostEstimate.estimate(estimateParam);
			
			if(estimateResult.isFlag()){
				successNum++;
				messageList.add("uuid:" + estimateParam.getBatchNumber() + "耗材费预估成功");
			} else {
				errorNum++;
				messageList.add("uuid:" + estimateParam.getBatchNumber() + "耗材费预估异常");
			}
	...
	public void estimateTest(){
		...
		if(estimateResult.isFlag()){
			successNum++;
			messageList.add("uuid:" + estimateParam.getBatchNumber() + "耗材费预估成功"+estimateResult.getMsg().get("msg"));
		} else {
			errorNum++;
			messageList.add("uuid:" + estimateParam.getBatchNumber() + "耗材费预估异常"+estimateResult.getMsg().get("msg"));
		}
