			...
			<table id= "table_content" class= "table table-hover table-striped" style= "table-layout: fixed;" >
		  		<tbody align= "center">
			  		...
			  		<tr>
			  			...
			  			<td class= "td_cs" nowrap="nowrap" title='${power.start_time}'>${power.start_time}</td>
			  			<td class= "td_cs" nowrap="nowrap">${power.store_name} </td>
			  			<td class= "td_cs" nowrap="nowrap">${power.warehouse_name}</td>
			  			<td class= "td_cs" nowrap="nowrap">这个字段没有</td>
			  			<td class= "td_cs" nowrap="nowrap">${power.qty_size}</td>
	...
	<script type="text/javascript">
		...
		function pageJump() {
			...
			// alert(param+"&page="+$("#pageIndex").val()+"&pageSize="+$("#pageSize").val());
		 	$.ajax({
					type: "POST",
		           	url:'${root}/control/storageExpendituresController/page.do?'+param+"&page="+$("#pageIndex").val()+"&pageSize="+$("#pageSize").val(),
		           	dataType: "text",
		           	data:'',
		    		cache:false,
