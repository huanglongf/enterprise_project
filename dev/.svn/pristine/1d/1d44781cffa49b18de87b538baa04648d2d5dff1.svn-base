	...
	<head lang="en">
		...
		$(function(){
			$('#uploadFile').ace_file_input({
				...
				, before_remove:function() {
					...
					loadingCenterPanel('${root }/control/orderPlatform/updExpectedFromDate/upd_exptime.do');
					return true;
				}
		...
		function upload(){
			if($("#uploadFile").val() == ""){
				...
				$.ajaxFileUpload({
					//用于文件上传的服务器端请求地址
					url: '${root}control/orderPlatform/updExpectedFromDate/updExptimeUpload.do',
					...
					success: function (result, status){
						if(result.result_code=="FIE"){
							...
							if(result.success_records>0){
								alert("文件上传成功！成功导入" + result.success_records + "条原始数据！");
							}else{
								alert("文件上传失败！");
							}
							if(result.failed_content!=''){
								alert(result.failed_content);
							}
			            }
						cancelLoadingStyle();
...
<body>
		...
		<div class="page-header"><h1 style='margin-left:20px'>批量更新</h1></div>	
