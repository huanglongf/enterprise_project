...
function cover(){
	...
	background.css('z-index', 1040 + (10 * $('.modal:visible').length == 0 ? 99999999 : $('.modal:visible').length));
	// 指定坐标
	background.css("height", window.screen.height);
	background.css("width", window.screen.width);
...
//	$("#background").append(div);
	...
	$("#loadingCircle").css("height", "100px");
	$("#loadingCircle").css("width", "100px");
	$("#loadingCircle").css("top", (window.screen.height-100)/2);
	$("#loadingCircle").css("left", (window.screen.width-100)/2);
...
function pCSSCascadeFuzzyQuery(upper, lower){
	...
	if(area_code == ""){
		...
		$.ajax({
			url: root+ "/control/orderPlatform/waybillMasterController/getArea.do",
			type: "post",
			data: {
				"area_code" : area_code
...
//		$(".m-list").attr('style','display:none');
	setTimeout("test();",200);
	// 交互界面高度固定
	$(".interface").height($(window).height() - 136);
	commonFixedTableTitle();
});

function commonFixedTableTitle(){
	//固定表头
	$("#sc_content").css("position","relative");
	$("#sc_content").append($("#float_banner_table").clone());
    var cloneDiv = $("#table_content thead").clone();
	var float_banner_table = $("#sc_content #float_banner_table");
	float_banner_table.hide();
	var float_banner_table_table = float_banner_table.find("table");
	float_banner_table.width($("#table_content").width());
	float_banner_table_table.width($("#table_content").width());
	float_banner_table_table.append(cloneDiv);
	float_banner_table.css("position","absolute");
	$("#sc_content").scroll(function(e){
		float_banner_table.show();
		float_banner_table.css("top",$(this).scrollTop());
	});
}
