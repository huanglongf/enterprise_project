...
<html>
	<head lang="en">
		...
		function pageJump() {
			 ...
			 $.ajax({
					type: "POST",
		          	url:'${root}control/orderPlatform/waybillMasterController/pagea.do?'+
		          			param+
		          			'&startRow='+
		          			$("#startRow").val()+
		          			'&endRow='+
		...
		function addlogisticsa() {
			$.post('${root}/control/orderPlatform/waybillMasterController/interceptingTime.do?',function(data){
	            if(data.data==1){
	            	loadingCenterPanel('${root }/control/orderPlatform/waybillMasterController/addlogisticsa.do');
	            }else{
	            	alert("目前已过截单时间，请在明天 8:00到18:00间下单！");
	             }
                });
		}
	...
	<body>
		<div class="page-header"><h1 style='margin-left:20px;'>门店运单</h1></div>
		...
		<div style= "height: 300px; overflow: auto; overflow: scroll; border: solid #F2F2F2 1px;"  id= "sc_content">
